...
struct Scanner {
   ...
   template <size_t N> array<int, N> nextArrayInt(int offset=0) const { array<int, N> r; rep(i, N) r[i] = nextInt(offset); return r; }
   ...
   pii nextPii() const { return make_pair(nextInt(), nextInt()); }
   pii nextPii(int offset1, int offset2) const { return make_pair(nextInt(offset1), nextInt(offset2)); }
