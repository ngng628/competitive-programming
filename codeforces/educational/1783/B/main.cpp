...
struct Solver {
   ...
   void greedy() {
      ...
      while (true) {
         ...
         rep (i, n) {
            rep (j, n) {
               int x = perm[n*i + j];
               if (i - 1 >= 0) st.insert(abs(x - perm[n*(i - 1) + j]));
               if (i + 1 < n) st.insert(abs(x - perm[n*(i + 1) + j]));
               if (j - 1 >= 0) st.insert(abs(x - perm[n*(i) + j - 1]));
               if (j + 1 < n) st.insert(abs(x - perm[n*(i) + j + 1]));
            }
         }
   ...
   bool judge(vvi mat) {
      ...
      rep (i, n) {
         rep (j, n) {
            int x = mat[i][j];
            if (i - 1 >= 0) st.insert(abs(x - mat[i - 1][j]));
            if (i + 1 < n) st.insert(abs(x - mat[i + 1][j]));
            if (j - 1 >= 0) st.insert(abs(x - mat[i][j - 1]));
            if (j + 1 < n) st.insert(abs(x - mat[i][j + 1]));
         }
      }

      cout << st.size() << endl;

      return st.size() == n*n - 1;
   }
};
