...
void Main(int p, int f, int cnt_s, int cnt_w, int s, int w)
{
    int cheep = 0, expensive = 0;
    int cnt_cheep = 0, cnt_expensive = 0;
    if (s < w) {
        cheep = s;
        cnt_cheep = cnt_s;
        expensive = w;
    ...
    //int ans = 0;
    //{
    //    int y = min(p / cheep, cnt_cheep);
    //    cnt_cheep -= y;
    //    int x = min((p - y*cheep) / expensive, cnt_expensive);
    //    cnt_expensive -= x;
    //    ans += x + y;
    //    //Print("プレイヤー:", x, y);
    //    //Print(x,"・",expensive,"+",y,"・",cheep,"=",x*expensive + y*cheep);
    //}
    //{
    //    int y = min(f / cheep, cnt_cheep);
    //    int x = min((f - y*cheep) / expensive, cnt_expensive);
    //    ans += x + y;
    //}

    int ans = 0;
    {
        int y = min((p+f) / cheep, cnt_cheep);
        cnt_cheep -= y;
        int x = min(((p+f) - y*cheep) / expensive, cnt_expensive);
        cnt_expensive -= x;
        ans += x + y;
    }
