...
struct LCA {
   ...
   int distanceBetween(int u, int v) { 
      return dist[u] + dist[v] - 2*dist[ancestor(u, v)];
   }

   bool is_on_path(int u, int v, int a) {
      return distanceBetween(u, a) + distanceBetween(a, v) == distanceBetween(u, v);
   }
};
...
struct Solver {
   ...
   void solve() {
      ...
      vi depth(n);
      rep (i, n) {
         depth[i] = lca.distanceBetween(root, i);
      }

      auto Q = sc.nextInt();
      rep (Q) {
         auto K = sc.nextInt();
         auto a = sc.nextVi(K, -1);
         rsort(a, [&depth](int u, int v) {
         ...
         int s = a[0];
         pair<int, int> top = { -1, oo };
         rep (i, K) if (chmin(top.second, depth[a[i]])) top.first = i;
         vi rem;
         rep (i, K) {
            if (lca.ancestor(s, a[i]) != a[i]) {
               rem.push_back(a[i]);
            }
         }
         ...
         if (rem.empty()) {
            ...
            rsort(rem, [&depth](int u, int v) {
               return depth[u] < depth[v];
            });
            s = rem[0];
            vi rem2;
            for (int i : rem) {
               if (lca.ancestor(s, i) != i) {
                  rem2.push_back(i);
               }
            }
            puts(rem2.empty() and depth[lca.ancestor(rem[0], a[0])] <= top.second ? "YES" : "NO");
         }
      }
   }
};
