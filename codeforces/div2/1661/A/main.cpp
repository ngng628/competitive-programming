...
using vvb = basic_string<vb>;
using vvvb = basic_string<vvb>;
template <typename T, size_t N> auto make_vector(vi& sizes, const T& x) { if constexpr (N == 1) { return vector(sizes[0], x); } else { int size = sizes[N - 1]; sizes.pop_back(); return vector(size, make_vector<T, N - 1>(sizes, x)); } }
template <typename T, size_t N> auto make_vector(int const(&sizes)[N], const T& x = T()) { vi s(N); rep (i, N) s[i] = sizes[N - i - 1]; return make_vector<T, N>(s, x); }
...
template <class F> struct Bind { F f; Bind(F &&f_) : f(std::forward<F>(f_)) {} template <class... Args> auto operator()(Args &&...args) const { return f(*this, std::forward<Args>(args)...); } };
struct Scanner {
   Scanner() = default;
   int nextInt(int offset = 0) const { char c = skip(); int r = c - '0'; int sgn = 1; if (c == '-') sgn = -1, r = gc() & 0xf; else if (c == '+') r = gc() & 0xf; while (not isspace(c = gc())) r = 10 * r + (c & 0xf); return sgn * r + offset; }
   ...
   tuple<int, int, vvi> nextGraph(int offset=-1) const { auto [n, m] = nextPii(); vvi g(n); rep (m) { auto [a, b] = nextPii(offset, offset); g[a].push_back(b); g[b].push_back(a); } return make_tuple(n, m, g); }
   tuple<int, int, vvi> nextDirectedGraph(int offset=-1) const { auto [n, m] = nextPii(); vvi g(n); rep (m) { auto [a, b] = nextPii(offset, offset); g[a].push_back(b); } return make_tuple(n, m, g); }
   tuple<int, int, vvi> nextTree(int offset=-1) const { int n = nextInt(); vvi g(n); rep (n - 1) { auto [a, b] = nextPii(offset, offset); g[a].push_back(b); g[b].push_back(a); } return make_tuple(n, n - 1, g); }
   tuple<int, int, vvi> nextDirectedTree(int offset=-1) const { int n = nextInt(); vvi g(n); rep (n - 1) { auto [a, b] = nextPii(offset, offset); g[a].push_back(b); } return make_tuple(n, n - 1, g); }
private:
...
class Random {
public:
   Random() : x(185973876), y(97070167), z(265786), w(34685849) {}
   ...
   [[nodiscard]] int nextInt(int a, int b) noexcept {
      uint64_t t = (x^(x<<11));
      x = y; y = z; z = w;
      w = (w^(w>>19))^(t^(t>>8));
      return w % (b - a) + a;
