...
struct Solver {
   auto read() {
      int n;
      cin >> n;
      vi miners(n),diamonds(n);
      int i = 0, j = 0;
      rep (_, 2*n) {
         int a, b;
         cin >> a >> b;
         if (a == 0) miners[i++] = abs(b);
         else diamonds[j++] = abs(a);
      }
      return tuple{n, miners, diamonds};
   }
   
   void solve() {
      auto [n, miners, diamonds] = read();
      sort(all(miners));
      sort(all(diamonds));
      float ans = 0.0;
      rep (i, n) ans += hypotl(miners[i], diamonds[i]);
      print(ans);
   }
};
