...
bool Main() {
	...
	do {
		...
		int i = 0, j = 1, k = 2;
		int sum = 0;
		while (i < n and j < n and k < n and b[i] > 0 and b[j] > 0 and b[k] > 0) {
			int m = min(backup[i], min(backup[j], backup[k]));
			backup[i] -= m;
			backup[j] -= m;
			backup[k] -= m;
			sum += m;
			if (backup[i] == 0) i = max(i, max(j, k)) + 1;
			if (backup[j] == 0) j = max(i, max(j, k)) + 1;
			if (backup[k] == 0) k = max(i, max(j, k)) + 1;
		}
		chmax(ans, sum);
