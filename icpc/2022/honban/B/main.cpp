...
struct Solver {
   Solver() = default;
   void solve(int n, vvi c) {
      constexpr int nil = oo;

      // 最初に捨てる
      rep (i, n) {
         rep (j, 3) rep (k, j + 1, 3) {
            if (c[i][j] == c[i][k]) c[i][j] = c[i][k] = nil;
         }
      }
      
      auto finished = [&](int i) {
         return all_of(all(c[i]), [&](int x){ return x == nil; });
      };
      ...
      while (not Ok()) {
         if (finished(i)) {
            ...
            if (now == nil) {
               ...
               // 一番小さいものを選ぶ
               int min_index = distance(c[now].begin(), min_element(all(c[now])));
               int hand = c[now][min_index];
               c[now][min_index] = nil;
               rep (j, 3) {
                  if (c[nxt][j] == nil) {
                     c[nxt][j] = hand;
