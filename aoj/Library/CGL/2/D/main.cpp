...
struct Line {
   ...
   [[nodiscard]] constexpr float distance_from(const Point& p) const {
      if (dir().dot(p - a) < EPS) return (p - a).length();
      else if ((-dir()).dot(p - b) < EPS) return (p - b).length();
      else return abs(dir().cross(p - a)) / dir().length();
   }

   [[nodiscard]] constexpr float distance_from(const Line& other) const {
      if (intersects(other)) return 0.0;
      float ans = distance_from(other.a);
      chmin(ans, distance_from(other.b));
      chmin(ans, other.distance_from(a));
      chmin(ans, other.distance_from(b));
      return ans;
   }
   ...
   [[nodiscard]] friend constexpr Point cross_point(const Line& s, const Line& t) {
      double d1 = s.dir().cross(t.dir());
      double d2 = s.dir().cross(s.b - t.a);
      if (eq(abs(d1), 0) and eq(abs(d2), 0)) return t.a;
      else return t.a + t.dir() * (d2 / d1);
   }
