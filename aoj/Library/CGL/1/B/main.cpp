...
[[nodiscard]] bool eq(float a, float b){ return abs(a - b) <= EPS; }
[[nodiscard]] float to_rad(float deg){ return deg * PI / 180.0; }
[[nodiscard]] float operator ""_deg(float deg){ return deg; }
[[nodiscard]] float to_deg(float rad){ return rad * 180.0 / PI; }
...
struct Line {
   ...
   // 方向ベクトル
   [[nodiscard]] constexpr Vec2 dir() const {
      return b - a;
   }
