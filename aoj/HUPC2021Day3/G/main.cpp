...
int32 main() {
   ...
   dist[sy][sx] = 0;
   heap.emplace(0, make_pair(sy, sx));
   while (not heap.empty()) {
      ...
      for (auto [dy, dx] : dydx4) {
         ...
         int cost = [&]() -> int {
            if (s[ny][nx] == '#') return oo;
            if (s[now.first][now.second] != '.') {
               if (to_dir(dy, dx) == s[now.first][now.second]) return dist[now.first][now.second];
               else return oo;
            }
            else return dist[now.first][now.second] + 1;
         }();
         if (dist[ny][nx] > cost) {
            dist[ny][nx] = cost;
   ...
   cout << (dist[gy][gx] >= oo ? -1 : dist[gy][gx]) << '\n';
}