...
static const int N_MAX = 300;
float dp[N_MAX+1][N_MAX+1][N_MAX+1];
bool visited[N_MAX+1][N_MAX+1][N_MAX+1];
...
int32_t main() {
   ...
   dp[0][0][0] = 0;
   visited[0][0][0] = true;
   auto rec = [&](auto self, int x, int y, int z) {
      if (visited[x][y][z]) return dp[x][y][z];
      visited[x][y][z] = true;
      float p = (float)(n-x-y-z) / (float)n;
      dp[x][y][z] = 1;
      if (z > 0) dp[x][y][z] += self(self, x, y, z-1) * (float)(z) / (float)n;
      if (y > 0) dp[x][y][z] += self(self, x, y-1, z+1) * (float)(y) / (float)n;
      if (x > 0) dp[x][y][z] += self(self, x-1, y+1, z) * (float)(x) / (float)n;
      return dp[x][y][z] /= 1 - p;
   };
