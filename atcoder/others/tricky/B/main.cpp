...
int32_t main() {
   ...
   while (t--) {
      ...
      if (a != 0) {
         float D = b*b - 4*a*c;
         if (b*b == 4*a*c) print(1, (float)-b/(2.0*a));
         else if (b*b < 4*a*c) print(0);
         else {
            pair<float, float> ans = {(float)(-b-sqrtl(D)) / (2.0*a), (float)(-b+sqrtl(D)) / (2.0*a)};
            print(2, min(ans.fi, ans.se), max(ans.fi, ans.se));
         }
      }
      else if (a == 0 and b == 0) {
         print(0);
