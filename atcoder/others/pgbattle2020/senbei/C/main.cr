...
que = Deque.new [{si, sj, up}]
dist = Array.new(h){ |i| Array.new(w){ |j| Array.new(4){ |dir| {i, j} == {si, sj} && dir == up ? 0_i64 : OO } } }
grid = Grid.dydx4(h, w, s)
until que.empty?
  i, j, dir = que.shift

  [1, 0, -1, 2].each do |delta|
    dir2 = (dir + delta) % 4
    ni = i + Grid::DYDX4[dir2][0]
    nj = j + Grid::DYDX4[dir2][1]
    unless grid.barred?(ni, nj)
      break if dist[ni][nj][dir2] != OO
      dist[ni][nj][dir2] = dist[i][j][dir].not_nil! + 1
      que << {ni, nj, dir2}
      break
    end
  end
