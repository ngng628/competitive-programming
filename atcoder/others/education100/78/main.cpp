...
using vvi = vec<vi>;
using vvvi = vec<vvi>;
constexpr long long INF = (1LL<<62)-(1LL<<31);
constexpr long double EPS = 1e-10;
constexpr long double PI = 3.1415926535897932385;
...
template <class Type>
Type query(const vec<vec<Type>>& sums, int x1, int y1, int x2, int y2) {
   x1--; y1--;
...
int main() {
   ...
   rep (y, M) {
      ...
      rep (x, N) {
         switch (s[x]) {
            case 'J': jungle[y][x] = 1; break;
            case 'O': ocean[y][x] = 1; break;
            case 'I': ice[y][x] = 1; break;
         }
      }
   }

   vvvi cs = { cumsum(jungle), cumsum(ocean), cumsum(ice) };

   while (K--) {
      int a, b, c, d;
      cin >> a >> b >> c >> d;
      print(query(cs[0], a, b, c, d),
            query(cs[1], a, b, c, d),
            query(cs[2], a, b, c, d)
      );
   }
