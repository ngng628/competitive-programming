...
bool can(const vector<string>& town, P pos) {
    if (InRange(pos.Y, (int)0, len(town)) and InRange(pos.X, (int)0, len(town[0]))) {
        if (town[pos.Y][pos.X] != 'X') {
            return true;
        }
    }
    return false;
...
signed main()
{
    ...
    vector<P> factorys(N + 1);
    rep (y, H) {
        rep (x, W) {
            if (town[y][x] == 'S') {
                factorys[0] = make_pair(y, x);
            }
            else if (int id = ctoi(town[y][x]); id != -1) {
                factorys[id] = make_pair(y, x);
            }
        }
    }

    int ans = 0;
    range_for (i, 0, len(factorys) - 1) {
        P goal = factorys[i + 1];
        ...
        while (not que.empty()) {
            ...
            if (now.Y == goal.Y and now.X == goal.X) {
                break;
            }
            for (auto [dy, dx] : dydx) {
                P nxt = make_pair(now.Y + dy, now.X + dx);
                if (dist[nxt] > 0) continue;
                if (not can(town, nxt)) continue;
                dist[nxt] = dist[now] + 1;
                que.push(nxt);
            }
        }
