...
struct Edge {
   Edge() = default;
   Edge(int t, int w, int ti) : to(t), dist(w), time(ti) {}
   int to;
   int dist;
   int time;
};
      ...
      for (int now = 0; now < v; ++now) {
         for (auto&& nxt : graph[now]) {
            if (((s >> nxt.to) & 1) == 0) {
               int d = nxt.dist;
               int t = nxt.time;
               int total = dp[s][now] + d;
               if (total > t) continue;
               if (dp[s | bit(nxt.to)][nxt.to] == total) {
                  cnt[s | bit(nxt.to)][nxt.to] += cnt[s][now];
               }
               if (dp[s | bit(nxt.to)][nxt.to] > total) {
                  dp[s | bit(nxt.to)][nxt.to] = total;
                  cnt[s | bit(nxt.to)][nxt.to] = cnt[s][now];
               }
            }
         }
