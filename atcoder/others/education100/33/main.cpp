...
signed main()
{
    ...
    rep (y, H) rep (x, W)
    {
        ...
        for (auto [dy, dx] : dydx)
        {
            if (InRange(y + dy, (int)0, H) and InRange(x + dx, (int)0, W))
            {
                if (s[y + dy][x + dx] == '.')
                {
                    G[y*W + x].pb((y+dy)*W + (x+dx));
                }
            }
        }
    }
    ...
    if (d[W*H - 1] == 0) cout << "-1" << endl;
    else cout << W*H - black - d[W*H - 1] << endl;
