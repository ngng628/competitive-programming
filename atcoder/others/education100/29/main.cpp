...
bool can(const vector<string>& maze, P pos) {
    if (InRange(pos.Y, (int)0, len(maze)) and InRange(pos.X, (int)0, len(maze[0]))) {
        if (maze[pos.Y][pos.X] == '.') {
            return true;
        }
    }
...
signed main()
{
    ...
    Cin(int, R, C);
    Cin(int, sy, sx);
    P start = make_pair(sy - 1 , sx - 1);
    Cin(int, gy, gx);
    P goal = make_pair(gy - 1, gx - 1);
