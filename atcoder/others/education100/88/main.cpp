...
struct Stone {
   int color;
   int length;
   Stone(int c, int l) : color(c), length(l) { assert(color == 0 or color == 1); }
   constexpr void flip() { color ^= 1; }
};
...
int32_t main() {
   ...
   repr (i, 1, n) {
      if ((i+1) & 1) {
         if (stones.back().color != c[i]) stones.eb(c[i], 1);
         else stones.back().length++;
      }
      else {
         if (stones.back().color != c[i]) {
            int m = len(stones);
            if (m == 1) stones.back().flip();
            else {
               stones[m-2].length += stones[m-1].length;
               stones.pop_back();
            }
         }
         stones.back().length++;
   ...
   for (auto& stone : stones) {
      if (stone.color == 0) ans += stone.length;
   }
   print(ans);
}