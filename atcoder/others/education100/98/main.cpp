...
int32_t main() {
   ...
   vi a(n + 1);
   reps (i, n) cin >> a[i];
   vvi table(n+1, vi(3, 0));
   reps (i, n) {
      table[i] = table[i-1];
      if (table[i-1][0] == a[i]) table[i][0]++;
      else if (table[i-1][1] == a[i]) table[i][1]++;
      else if (table[i-1][2] == a[i]) table[i][2]++;
   }

   mint ans = 1;
   reps (i, n) ans *= count(all(table[i-1]), a[i]);
   print(ans.val());
}
