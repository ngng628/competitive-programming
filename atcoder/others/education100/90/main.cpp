...
int32_t main() {
   int n, m;
   cin >> n >> m;
   vec<float> x(n + m), y(n + m), r(n);
   rep (i, n) cin >> x[i] >> y[i] >> r[i];
   rep (i, m) {
      cin >> x[n+i] >> y[n+i];
   }

   auto isOK = [&](float mid) -> bool {
      rep (i, n+m) {
         repr (j, i+1, n+m) {
            float dist = hypot(abs(x[i] - x[j]), abs(y[i] - y[j]));
            float r2 = (i >= n ? mid : r[i]) + (j >= n ? mid : r[j]);
            if (r2 > dist) return false;
         }
      }
      return true;
   };
   ...
   if (n == 0) drop(ok);
   print(min(*min_element(all(r)), ok));
}
