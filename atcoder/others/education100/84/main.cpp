...
int32_t main() {
   ...
   rep (i, n+2) repr (k, 1, i+2) imos[i][k] += imos[i][k-1];
   repr (i, 1, n+2) rep (k, i+1) imos[i][k] += imos[i-1][k];
   repr (i, 1, n+2) repr (k, 1, i+2) imos[i][k] += imos[i-1][k-1];

   int ans = 0;
   rep (i, n+2) rep (k, i+2) if (imos[i][k] != 0) ans++;
   print(ans );
}
