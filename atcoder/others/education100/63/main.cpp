...
int32_t main() {
   ...
   vvi A(n, vi(n));
   cin >> A;
   rep (i, n) rep (k, n) rep (m, n) {
      if (A[i][k] + A[k][m] < A[i][m]) drop(-1);
   }

   vvi M(A);
   rep (i, n) rep (k, n) rep (m, n) {
      if (i == k or k == m or m == i) continue;
      if (A[i][k] + A[k][m] == A[i][m]) M[i][m] = INF;
   }

   int ans = 0;
   rep (i, n) repr (k, i+1, n) {
      if (M[i][k] != INF) ans += M[i][k];
   }
   print(ans);
}
