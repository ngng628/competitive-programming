...
bool can(const vector<vector<int>>& c, const int y, const int x)
{
    if (InRange(y, (int)0, h) and InRange(x, (int)0, w))
    {
        if (c[y][x] == 1)
        {
            return true;
...
void dfs(vector<vector<int>> c, const int y, const int x, int dist)
{
    if (!can(c, y, x))
    {
        return;
