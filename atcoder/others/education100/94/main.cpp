...
struct Cake {
   int width, depth, area, turn;
   Cake(int w, int d, int t) : width(w), depth(d), area(w*d), turn(t) {}
};

bool operator <(const Cake& a, const Cake& b) {
   if (a.turn == b.turn) return a.area < b.area;
...
int32_t main() {
   ...
   while (cin >> n >> w >> d, n|w|d) {
      ...
      reps (id, n) {
         ...
         int k = 1, diff = s;
         for (; diff > 0; ++k) {
            diff -= (k % 2) ? piece.width : piece.depth;
         }
         diff *= -1;

         if (k % 2) {
            cakes.eb(piece.width, piece.depth - diff, id);
            cakes.eb(piece.width, diff, id);
         }
         else {
            cakes.eb(piece.width - diff, piece.depth, id);
            cakes.eb(diff, piece.depth, id);
         }
      ...
      vi ans;
      for (Cake& cake : cakes) ans.pb(cake.area);
      sort(all(ans));
      rep (i, len(ans)) {
         if (i != 0) cout << " ";
         cout << ans[i];
