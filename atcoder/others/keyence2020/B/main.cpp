...
// binary_search の ok が求めたかった値そのもの
...
int main()
{
    ...
    for (int32 i : step(N))
    {
        X[i] = ReadInt();
        L[i] = ReadInt();
        ps.push_back(P(X[i] - L[i], X[i] + L[i]));
    }
    ...
    for (int32 i = 0; i+1 < len(ps); ++i)
    {
        if (ps[i].first == ps[i+1].first)
        {
            int32 tmp1 = ps[i].second;
            int32 tmp2 = ps[i+1].second;
            if (tmp1 < tmp2)
            {
                ps.remove_at(i+1);
                ps[i+1].first = ps[i+1].second = 0;
            }
            else
            {
                ps[i].first = ps[i].second = 0;
            }
            --i;
        }
    }
