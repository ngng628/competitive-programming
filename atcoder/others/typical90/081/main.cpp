...
// キーワード : とりあえずソートしてみよう
// 尺取り法できない？
//  - もし、a[right].fi - a[left] <= K && abs(min(a.left(), a.right()) - max(a.left(), a.right())) <= K
// これは嘘で、A_i = A_j があると詰む場合がある
...
int32_t main() {
   ...
   rep (i, N) table[A[i]][B[i]]++;

   auto sums = cumsum(table);
   int ans = 0;
   reps (i, MAX - K - 1) {
      reps (j, MAX - K - 1) {
         chmax(ans, query(sums, i, i + K + 1, j, j + K + 1));
      }
   }
