...
int32_t main() {
   ...
   // dp[v][S] := 部分木 v について、状態が S なのは何通り？
   auto Only = [](int c) -> int {
      if (c == 'a') return 1;
      else return 2;
   };

   vec<vec<Fp>> dp(n, vec<Fp>(4, 0));
   auto dfs = [&](auto&& self, int root, int pre) -> void {
      Fp t1 = 1, t2 = 1;
      for (int child : graph[root]) {
         if (child == pre) continue;

         self(self, child, root);
         t1 *= (dp[child][Only(c[root])] + dp[child][Only('a') | Only('b')]);
         t2 *= (dp[child][Only('a')] + dp[child][Only('b')] + 2*dp[child][Only('a') | Only('b')]);
      }

      dp[root][Only(c[root])] = t1;
      dp[root][Only('a') | Only('b')] = t2 - t1;
   };

   dfs(dfs, 0, -1);

   print(dp[0][Only('a') | Only('b')]);
}