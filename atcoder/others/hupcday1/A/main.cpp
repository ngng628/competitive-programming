...
int32_t main() {
   ...
   auto f = [&](int i1, int i2, char c) -> int {
      int res = 0;
      rep (i, i1, i2) {
         rep (j, w) {
            if (s[i][j] == c) res += 1;
   ...
   auto g = [&](int j1, int j2, char c) -> int {
      int res = 0;
      rep (j, j1, j2) {
         rep (i, h) {
            if (s[i][j] == c) res += 1;
   ...
   reps (i, h) {
      ...
      int top = i * w;
      int bottom = h*w - top;
      chmin(ans, top - f(0, i, 'o') + bottom - f(i, h, 'x'));
      chmin(ans, top - f(0, i, 'x') + bottom - f(i, h, 'o'));
   }
   reps (j, w) {
      if (j == w) break;
      int top = j * h;
      int bottom = h*w - top;
      chmin(ans, top - g(0, j, 'o') + bottom - g(j, w, 'x'));
      chmin(ans, top - g(0, j, 'x') + bottom - g(j, w, 'o'));
   }
