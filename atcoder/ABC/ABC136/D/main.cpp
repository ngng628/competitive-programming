...
struct Code {
   Code() = default;
   Code(int c, int l) : ch(c), length(l) {}
   char ch;
   int length;
};
vec<Code> RunLengthEncoding(const string& s) {
   vec<Code> res;
   for (const auto& e : s) {
      if (res.empty() or res.back().ch != e) res.eb(e, 1);
      else res.back().length++;
   }
   return res;
}
...
int32 main() {
   ...
   int cur = 0;
   rep (i, len(code)) {
      if (i == 0 and code[i].ch == 'L') {
         ans[i] = code[i].length;
         cur += code[i].length;
      }
      else if (i == len(code) - 1 and code[i].ch == 'R') {
         ans.back() = code[i].length;
         cur += code[i].length;
      }
      else {
         int n = code[i].length;
