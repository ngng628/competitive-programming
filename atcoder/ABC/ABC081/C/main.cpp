...
int main()
{
    int32 N = ReadInt(), K = ReadInt();
    Array<int32> A(N);
    A.each([](int32& a){ a = ReadInt() - 1; });

    Array<int32> memo(N, 0);
        Debug << "memo : " << memo;
    A.each( [&](int32 a) { Debug << "ai : " << a; memo[a]++; } );
        Debug << "memo : " << memo;
    ...
    else
    {
        memo.sort();
        Debug << "memo : " << memo;
        Debug << "cnt - K : " << len(memo) - K;
        int32 s = 0;
        for (int32 i : step(len(memo) - K))
        {
            s += memo[i];
        }
        Print << s;
