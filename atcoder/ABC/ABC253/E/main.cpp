...
int32 main() {
   auto [n, m, K] = sc.nextAi<3>();

   // mint g = 0;
   // vi emp;
   // Bind([&](auto&& self, vi a) -> void {
   //    if (len(a) == n) {
   //       bool ok = true;
   //       rep (i, n - 1) {
   //          if (abs(a[i] - a[i + 1]) < K) {
   ...
   dp[0] = 0;
   rep (i, 1, 10) dp[m + i] = 0;
   StaticRangeSum<mint> cumsum(dp);
   rep (i, 1, n) {
      vm dp2(m + 10, 0);
      reps (j, m) {
         if (K == 0) {
            dp2[j] += cumsum.get(1, m + 1);
         }
         else {
            if (j - K >= 1) {
               int lo = 1, hi = j - K;  // [lo, hi]
               dp2[j] += cumsum.get(lo, hi + 1);
            }
            if (j + K <= m) {
               int lo = j + K, hi = m;  // [lo, hi]
               dp2[j] += cumsum.get(lo, hi + 1);
            }
         }
      }
