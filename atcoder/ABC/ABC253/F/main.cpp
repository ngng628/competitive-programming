...
template<class T> using DynamicRangeSum = atcoder::fenwick_tree<T>;

int32 main() {
   constexpr int nil = -oo;
   auto [n, m, Q] = sc.nextAi<3>();
   vec<array<int, 4>> queries(Q);
   for (auto& [t, i1, i2, x] : queries) {
      t = sc.nextInt();
      if (t == 1) {
         i1 = sc.nextInt(-1);
         i2 = sc.nextInt();
   ...
   rep (q, Q) {
      ...
      if (t == 2) {
         ...
         int index = latest[i];
         indices[index].push_back(j);
      }
   }

   map<int, map<int, int>> S;
   {
      DynamicRangeSum<int> cumsum(m + 1);
      rep (q, Q) {
         int t = queries[q][0];
         if (t == 1) {
   ...
   {
      DynamicRangeSum<int> cumsum(m + 1);
      vi line(n, 0);  // i 行目は何にセットされている？
      vi last(n, -1);  // i 行目は何にセットされている？
      rep (q, Q) {
         int t = queries[q][0];
         if (t == 1) {
            int l = queries[q][1], r = queries[q][2];
            ...
            int Sqj = cumsum.sum(0, j + 1);
            int now = last[i];
            int ans = line[i] + Sqj - S[now][j];
            cout << ans << endl;
         }
      }
   }
