...
int32_t main() {
   int n;
   cin >> n;
   vec<Restaurant> restaurants(n);
   for (auto& restaurant : restaurants) cin >> restaurant.name >> restaurant.score;

   vi order(n);
   iota(all(order), 0);
   sort(all(order), [&restaurants](int i, int j){ return restaurants[i] < restaurants[j]; });
