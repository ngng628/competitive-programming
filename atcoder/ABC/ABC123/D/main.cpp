...
int32 main() {
   int x, y, z, K;
   cin >> x >> y >> z >> K;
   array<deque<int>, 3> abc;
   abc[0].resize(x), abc[1].resize(y), abc[2].resize(z);
   rep (i, 3) {
      for (auto& e : abc[i]) cin >> e;
      rsort(abc[i]);
   }
   ...
   rep (i, x) {
      rep (j, y) {
         sums.push_back(abc[0][i] + abc[1][j]);
      }
   }
   sort(sums);

   int Kth_sum = binary_chop(0LL, oo, [&](int d) {
      // ab + c >= d を満たすのは何通り？
      // ab >= d - c
      int cnt = 0;
      rep (k, z) {
         cnt += distance(lower_bound(all(sums), d - abc[2][k]), sums.end());
      }
      return cnt >= K;
   });
   ...
   rep (i, x) {
      rep (j, y) {
         rep (k, z) {
            int sum = abc[0][i] + abc[1][j] + abc[2][k];
            if (sum >=  Kth_sum) ans.pb(sum);
            else break;
         }
      }
   }
