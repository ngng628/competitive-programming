...
int32 main() {
   ...
   rep (i, n) {
      rep (j, i + 1, n) {
         // 変化量
         int dx1 = xy[j].first - xy[i].first;
         int dy1 = xy[j].second - xy[i].second;
         ...
         rep (k, n) if (k != i and k != j) {
            int dx2 = xy[k].first - xy[i].first;
            int dy2 = xy[k].second - xy[i].second;
            if (dx2 * dy1 == dx1 * dy2) {
               st2.insert(k);
            }
         }

         pii target = make_pair(*st2.begin(), *next(st2.begin()));
         if (len(st2) >= K and not st.count(target)) {
            st.insert(target);
         }
      }
   }
