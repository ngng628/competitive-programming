...
struct Query {
   Query() = default;
   Query(int _l, int _r, int _x, int _index)
      : l(_l)
      , r(_r)
      , x(_x)
      , index(_index)
...
int32 main() {
   ...
   auto Q = sc.nextInt();
   auto _queries = sc.nextVecAi<3>(Q, -1);
   ...
   vec<Query> queries(Q);
   rep (i, Q) {
      queries[i] = Query(_queries[i][0], _queries[i][1] + 1, _queries[i][2], i);
   }
   sort(queries, [](const Query& a, const Query& b) {
      return a.x < b.x;
   });
