...
# endif
    ...
    struct Point
    {
        ...
        constexpr Point(int64 _x, int64 _y) noexcept : x(_x), y(_y) {}
        constexpr int64 manhattanDistanceFrom(const Point& other) const noexcept
        {
...
int main()
{
    ...
    Point p0(1, 1);
    Array<int64> ns = divisor(N);

    if (ns.size() & 1) // 平方数
    {
        int64 sns = ns.size();
        int64 ib = sns/2;
        Point p1(ns[ib], ns[ib]);
        Print << p0.manhattanDistanceFrom(p1);
        return 0;
    }
    else
    {
