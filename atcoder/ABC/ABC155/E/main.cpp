...
int32 main() {
   ...
   auto dp = make_vector({ n + 1, 2 }, oo);
   enum Ordering { Equal, More };
   dp[0][Equal] = 0;
   dp[0][More] = 1;

   rep (i, n) {
      dp[i + 1][Equal] = min(dp[i][Equal] + s[i], dp[i][More] + 10 - s[i]);
      dp[i + 1][More] = min(dp[i][Equal] + s[i] + 1, dp[i][More] + 10 - s[i] - 1));
   }

   cout << dp[n][Equal] << endl;
}
