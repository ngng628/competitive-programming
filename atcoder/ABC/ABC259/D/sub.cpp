...
int32 main() {
   auto n = sc.nextInt();
   auto [sx, sy, tx, ty] = sc.nextAi<4>();
   vec<array<int, 3>> circles(n);
   ...
   using Circle = array<int, 3> ;
   auto intersect(Circle cir, int x, int y) {
      auto [a, b, r] = cir;
      return (x - a)*(x - 1) + (y - b)*(y - b) == r*r;
   };

   int s = -1;
   vvi graph(n);
   rep (i, n) {
      if (intersect(circles[i], sx, sy)) {
         s = i;
      }
      rep (j, n) {
         auto p1 = cir[i];
         ...
         int d = dx*dx + dy*dy;
         if (not(d > (p1[2] + p2[2])**2 || d < (p1[2] - p2[2])**2)) {
            graph[i].push_back(j);
         }
      }
   }
