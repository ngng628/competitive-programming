...
template <class T, class U> using HashMap = __gnu_pbds::gp_hash_table<T, U>;
# ifndef ONLINE_JUDGE
...
int32 main() {
   ...
   HashMap<int, MinHeap<int>> heaps;
   rep (i, n) {
      rep (j, m[i]) {
         heaps[p[i][j]].push(e[i][j]);
         if (len(heaps[p[i][j]]) > 2) heaps[p[i][j]].pop();
      }
   }

   HashMap<int, array<int, 2>> maxs;
   for (auto [p, _] : heaps) {
      maxs[p][0] = heaps[p].top();
      heaps[p].pop();
      if (not heaps[p].empty()) {
         maxs[p][1] = heaps[p].top();
         swap(maxs[p][0], maxs[p][1]);
         heaps[p].pop();
      }
      else {
         maxs[p][1] = -oo;
   ...
   rep (i, n) {
      ...
      rep (j, m[i]) {
         int pp = p[i][j];
         int ee = e[i][j];
         if (ee == maxs[pp][0] and ee != maxs[pp][1]) {
            seq[pp] = 1;
         }
      }
      st.insert(seq);
