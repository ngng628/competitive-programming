...
int32 main() {
   ...
   set<int> leaves = { takahashi };
   auto dfs = [&](auto&& Recall, int subroot, int par) -> void {
      bool is_leaf = true;
   ...
   for (auto leaf : leaves) {
      int anc = lca.query(leaf, takahashi);
      if (lca.get_dist(anc, takahashi) < lca.get_dist(anc, aoki)) {
         chmax(ans, lca.get_dist(leaf, aoki) - 1);
      }
      else {
         chmax(ans, lca.get_dist(takahashi, aoki) / 2);
      }
   }
