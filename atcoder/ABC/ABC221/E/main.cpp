...
using DynamicRangeSum = atcoder::fenwick_tree<mint>;

vi compressed(const vi& a) {
   vi b = a;
   sort(all(b));
   b.erase(unique(all(b)), b.end());
   vi res(len(a));
   rep (i, len(a)) res[i] = distance(b.begin(), lower_bound(all(b), a[i]));
   return res;
}

int32 main() {
   auto n = sc.nextInt();
   auto a = compressed(sc.nextVi(n));

   mint ans = 0;
   DynamicRangeSum cumsum(n);
   rep (r, 1, n) {
      cumsum.add(a[r - 1], (2_fp).pow(r - 1).inv());
      ans += (2_fp).pow(r - 1) * cumsum.sum(0, a[r] + 1);
   }
