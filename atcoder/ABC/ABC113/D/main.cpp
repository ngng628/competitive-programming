...
# include <nglib/bitoperations.hpp>
# endif
# ifdef ngng628_library
using mint = atcoder::modint1000000007;
istream& operator >>(istream& is, mint& r){ int t; is >> t; r = t; return is; }
ostream& operator <<(ostream& os, const mint& r){ return os << r.val(); }
mint operator"" _mod_1000000007(unsigned long long n) { return n; }
using vm = vec<mint>;
using vvm = vec<vm>;
using vvvm = vec<vvm>;

int32 main() {
   auto [h, w, goal] = sc.nextAi<3>({ 0, 0, -1 });

   auto dp = make_vector({ h + 1, w }, 0_mod_1000000007);
   dp[0][0] = 1;

   rep (i, h) {
      rep (start, w) {
         rep (mask, BitOperations::Bit(w - 1)) {
            vb S(w - 1, false);
            rep (i, w - 1) S[i] = BitOperations::Stand(mask, i);

            if (auto is = iota(w - 1); any_of(all(is), [&](int idx){ return S[idx] and S[idx + 1]; })) {
               continue;
            }

            if (start != w - 1 and S[start]) dp[i + 1][start + 1] += dp[i][start];
            else if (start != 0 and S[start - 1]) dp[i + 1][start - 1] += dp[i][start];
            else dp[i + 1][start] += dp[i][start];
         }
      }
   }
