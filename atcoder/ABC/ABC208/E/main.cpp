...
int32_t main() {
   ...
   vec<vec<map<int, int>>> dp(2, vec<map<int, int>>(2));
   enum Ordering { Less, Equal };
   dp[0][0][1] = 1;
   rep (i, n) {
      const int ni = ctoi(N[i]);
      vec<map<int, int>> dp2(2, vec<map<int, int>>(2));
      for (int ord : { Less, Equal }) {
         for (int leading_zero : { false, true }) {
            for (int k : dp[ord][leading_zero]) {
...
template<class T> auto max(T& v){ return *max_element(all(v)); }
template<class T> auto min(T& v){ return *min_element(all(v)); }
template<class T> auto lower_bound(T& v){ return lower_bound(all(v)); }
template<class T> auto upper_bound(T& v){ return upper_bound(all(v)); }
