...
n, m = ints
pars = ints -1
xy = (1..m).map { x, y = ints; {x - 1, y} }

hoken = Array.new(n) { -1_i64 }
xy.each do |x, y|
  chmax(hoken[x], y)
end

graph = Array.new(n) { [] of Int64 }
pars.each_with_index(1) do |pi, i|
  graph[i] << pi
  graph[pi] << i.to_i64
end

que = Deque({Int64, Int64}).new
que << {0_i64, -1_i64}
cache = Array.new(n) { -1_i64 }
until que.empty?
  now, par = que.shift
  chmax(cache[now], hoken[now])
  graph[now].each do |to|
    next if to == par
    chmax(cache[to], cache[now] - 1)
    que << {to, now}
  end
end

ans = cache.count { |it| it >= 0 }
puts ans
