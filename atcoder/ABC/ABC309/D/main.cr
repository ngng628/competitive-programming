...
n1, n2, m = ints
graph1 = NgLib::BfsGraph.new(n1)
graph2 = NgLib::BfsGraph.new(n2)
m.times do
  u, v = ints -1
  if 0 <= u && u < n1 && 0 <= v && v < n1
    graph1.add_edge(u, v, directed: false)
  else
    graph2.add_edge(u - n1, v - n1, directed: false)
  end
end

d1 = graph1.shortest_path(start: 0).max
d2 = graph2.shortest_path(start: (n1 + n2 - n1).abs - 1).max
