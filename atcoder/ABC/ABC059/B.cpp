...
int main()
{
    const String A = ReadWord();
    const String B = ReadWord();
    ...
    if (A.length() == B.length()) // 桁数が同じだったら
    {
        for (auto& i : step(A.length()))
        {
            int32 a = CharToInt32(A[i]);
            int32 b = CharToInt32(B[i]);
            if (a > b)
            {
                Print << "GREATER";
                return 0;
