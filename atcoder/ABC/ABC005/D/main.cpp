...
struct StaticRangeSum2D {
   ...
   T operator ()(int y1, int y2, int x1, int x2) const { return get(y1, y2, x1, x2); }
   ...
   rep (i, n) {
      rep (j, n) {
         reps (w, n) {
            reps (h, n) {
               int S = w*h;
               int d = cumsum(i, min(i + h, n), j, min(j + w, n));
               chmax(maximums[S], d);
            }
         }
      }
