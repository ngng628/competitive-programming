...
int32_t main() {
   ...
   for (int e : A) tako[e]++;
   vi customers(1001, 0);
   for (int e : B) customers[e]++;

   priority_queue<pii, vec<pii>, greater<pii>> pq; // {time, amount}
   for (int t = 0; t <= 1000; ++t) {
      if (tako[t]) pq.emplace(t, tako[t]);
      rep (_, customers[t]) {
         while (true) {
            if (pq.empty()) drop("no");
            auto now = pq.top();
            if (t - now.fi > T) pq.pop();
            else break;
         }
         auto now = pq.top();
         pq.pop();
         if (now.se - 1 >= 1) pq.emplace(now.fi, now.se - 1);
      }
   }
   print("yes");
}