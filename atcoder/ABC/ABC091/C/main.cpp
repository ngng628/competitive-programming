...
auto read() {
   ...
   sort(rall(ab), [](pii a, pii b) { if (a.se == b.se) return a.fi < b.fi; else return a.se < b.se; });
   sort(all(cd));
   return make_tuple(n, ab, cd, red, blue);
}

int32_t main() {
   auto [n, ab, cd, red, blue] = read();
   auto cmp = [](pii a, pii b){ return a.fi < b.fi and a.se < b.se; };
   int cnt = 0;
   rep (i, n) {
      rep (k, n) {
         if (cmp(ab[k], cd[i]) and ab[k].fi != INF and ab[k].se != INF) {
            cnt++;
            ab[k].fi = ab[k].se = INF;
            break;
         }
