...
int32 main() {
   ...
   dp[0][Equal] = 1;
   rep (i, n) {
      const int Li = ctoi(L[i]);
      for (auto ord : { Less, Equal }) {
         for (int da : {0, 1}) {
            for (int db : {0, 1}) {
               int d = da + db;
               int ord2 = ord;

               if (not (0 <= d and d <= 1)) continue;

               if (ord == Equal and d > Li) continue;
               if (d < Li) ord2 = Less;

               dp[i + 1][ord2] += dp[i][ord];
            }
         }
      }
   }

   cout << dp[n][Less] + dp[n][Equal] << endl;
}
