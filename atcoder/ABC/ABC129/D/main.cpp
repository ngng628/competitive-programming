...
int32 main() {
   ...
   vvi left_side(h, vi(w));
   rep (i, h) {
      int j = 0;
      while (j < w) {
         int l = j - 1;
         while (j < w and s[i][j] == '.') left_side[i][j++] = l;
         while (j < w and s[i][j] == '#') j++;
      }
   }

   vvi right_side(h, vi(w));
   rep (i, h) {
      int j = w - 1;
      while (j >= 0) {
         int r = j + 1;
         while (j >= 0 and s[i][j] == '.') right_side[i][j--] = r;
         while (j >= 0 and s[i][j] == '#') j--;
      }
   }
   ...
   rep (j, w) {
      ...
      while (i < h) {
         int t = i - 1;
         while (i < h and s[i][j] == '.') top_side[i++][j] = t;
         while (i < h and s[i][j] == '#') i++;
      }
   }

   vvi bottom_side(h, vi(w));
   rep (j, w) {
      int i = h - 1;
      while (i >= 0) {
         int b = i + 1;
         while (i >= 0 and s[i][j] == '.') bottom_side[i--][j] = b;
         while (i >= 0 and s[i][j] == '#') i--;
      }
   }
   ...
   rep (i, h) {
      rep (j, w) {
         if (s[i][j] == '#') continue;
         int l = left_side[i][j];
         int r = right_side[i][j];
         int t = top_side[i][j];
         int b = bottom_side[i][j];
         chmax(ans, r - l + b - t - 3);
      }
   }
