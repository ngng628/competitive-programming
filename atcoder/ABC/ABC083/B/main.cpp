...
# endif
    ...
    namespace detail
    {
        struct DebugBuffer
        {
            ...
            ~DebugBuffer() { if (formatData) std::cerr << "[Debug]" << formatData->string << '\n'; }
            template <class Type> DebugBuffer& operator <<(const Type& x) { Formatter(*formatData, x); return *this; }
        };

        struct Debug_impl
        {
            void operator()(const String& text) const { std::cerr << "[Debug]"<< text << '\n'; }
            template <class... Args> void operator()(const Args&... args) const { operator()(Format(args...)); }
            template <class Type, class = decltype(Formatter(std::declval<FormatData&>(), std::declval<Type>()))>
...
int main()
{
    int32 N = ReadInt(), A = ReadInt(), B = ReadInt();

    int32 ans = 0;
    for (int32 i : Range(1, N))
    {
        int32 sum = 0;
        for (int32 k = 0; k < GetDigit(i); ++k)
        {
            sum += GetDigit(i, k);
        }
