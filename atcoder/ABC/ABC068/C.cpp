...
# endif
    ...
    template <class Type>
    constexpr int32 len(Array<Type> ary) noexcept
    {
        return ary.size();
    }
...
int main()
{
    ...
    Array<int64> dis(N+1, -1);
    dis[0] = 0;
    queue<int64> que;
    que.push(0);

    cerr << "dis[3] : " << dis[3] << endl;
    cerr << "len(dis) : " << len(dis) << endl;
    while (!que.empty())
    {
        int64 now = que.front(); que.pop();
        int64 sz = len(graph[now]);
        for (int64 i : step(sz))
        {
            int64 next = graph[now][i];
