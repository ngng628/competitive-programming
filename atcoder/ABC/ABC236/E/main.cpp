...
T bisect(T ac, T wa, const F& f, int ts = 80) {
   while (ts--) {
      T wj = (ac + wa) / 2;
      (f(wj) ? ac : wa) = wj;
   }
...
int32 main() {
   ...
   float ave = bisect(1.0, 2e9, [&](float K) {
      vec<vec<float>> dp(n + 1, vec<float>(2, 0.0));
      rep (i, n) {
         dp[i + 1][0] = dp[i][1];
         dp[i + 1][1] = max(dp[i][0], dp[i][1]) + float(a[i]) - K;
      }
      return max(dp[n]) >= 0.0;
   });

   int med = binary_chop(int(0), int(8e18), [&](int K) {
      vvi dp(n + 1, vi(2, 0.0));
      rep (i, n) {
         dp[i + 1][0] = dp[i][1];
         dp[i + 1][1] = max(dp[i][0], dp[i][1]) + (a[i] >= K ? 1 : -1);
      }
      return max(dp[n]) > 0;
   });
