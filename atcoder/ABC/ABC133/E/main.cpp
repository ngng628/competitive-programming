...
int32 main() {
   ...
   auto isroot = [](int v) { return v == 0; };

   mint ans = K;
   Fix([&](auto&& Recall, int v, int p) -> void {
      const int n_children = isroot(v) ? len(graph[v]) : len(graph[v]) - 1;
      ans *= isroot(v) ? npermk(K - 1, n_children) : npermk(K - 2, n_children);
      for (int c : graph[v]) {
         if (c == p) continue;
         Recall(c, v);
      }
