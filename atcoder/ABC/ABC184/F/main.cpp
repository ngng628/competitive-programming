...
vec<typename iterator_traits<InputIterator>::value_type>
binary_choose(InputIterator first, InputIterator last, Func f, typename iterator_traits<InputIterator>::value_type init) {
   using value_type = typename std::iterator_traits<InputIterator>::value_type;
   const int n = distance(first, last);
   vec<value_type> a(first, last);
   vec<value_type> res(1 << n);
   rep (mask, 1 << n) {
      value_type acc = init;
      rep (i, n) {
         if ((mask >> i) & 1) acc = f(acc, a[i]);
      }
      res[mask] = acc;
   }
   return res;
...
int32 main() {
   ...
   pair<vi, vi> sums = make_pair(
      binary_choose(a.begin(), a.begin() + n / 2, plus<int>(), 0),
      binary_choose(a.begin() + n / 2, a.end(), plus<int>(), 0)
   );
   sums.second.push_back(-oo);
   sort(sums.first);
   sort(sums.second);

   int ans = 0;
   for (auto sum : sums.first) if (sum <= T) chmax(ans, sum);
   for (auto sum : sums.second) if (sum <= T) chmax(ans, sum);
   for (auto s1 : sums.first) {
      auto it = upper_bound(sums.second, T - s1) - 1;
      if (s1 + *it <= T) chmax(ans, s1 + *it);
   }
