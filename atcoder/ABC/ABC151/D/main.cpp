...
bool is_wall(const vector<string>& S, int y, int x)
{
    return !InRange(y, 0LL, H) or !InRange(x, 0LL, W) or S[y][x] == '#';
}

bool is_road(const vector<string>& S, int y, int x)
{
    return S[y][x] == '.';
}

int dfs(const vector<string>& S, int y, int x)
{
    int maxi = 0;
    queue<P> que;
    ...
    while (not que.empty())
    {
        ...
        chmax(maxi, dist[now.Y][now.X]);
        rep (i, 4)
        {
            P nxt = {now.Y + dy[i], now.X + dx[i]};
            if (is_wall(S, nxt.Y, nxt.X)) continue;
