...
static const Array<int32> DY = {0, 0, -1, 1};
static const Array<int32> DX = {1, -1, 0, 0};
...
void fun(Array<String> S, int64 cnt, int32 y, int32 x, int32 gy, int32 gx)
{
    for (int32 i : step(4))
    {
        int32 dy = DY[i];
        ...
        else if (S[ny][nx] == 'v' or S[ny][nx] == '#')
        {
            continue;
        }
        S[ny][nx] = 'v';
        fun(S, cnt+1, ny, nx, gy, gx);
        if (ny == gy and nx == gx)
        {
            S[ny][nx] = 'v';
            if (S == ANS)
    ...
    for (int32 sy : step(H))
    {
        for (int32 sx : step(W))
        {
            for (int32 gy : step(H))
            {
                for (int32 gx : step(W))
                {
                    if (abs(sy - gy) + abs(sx - gx) < (H-1)+(W+1)) { continue; }
                    if (S[sy][sx] == '#') { continue; }
                    if (S[gy][gx] == '#') { continue; }
                    if (sy == 0 and sx == 0 and gy == H-1 and gx == W-1) { Debug << "OK"; }
                    ans = INT64_MAX;
                    if (sy == gy and sx == gx) { chmax(ans, (int64)0); continue; }
                    Array<String> tmp = S;
                    tmp[sy][sx] = 'v';
                    int64 cnt = 0;
                    fun(tmp, cnt, sy, sx, gy, gx);
                    if (ans == H*W)
                    {
                        Print << ans;
                        return 0;
