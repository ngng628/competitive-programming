...
int Distance2(int x, int y, int s, int t)
{
    return Square(x - s) + Square(y - t);
}
...
bool isOK(float r) {
    ...
    rep (i, N)
    {
        if (Square(x[i] - gx) + Square(y[i] - gy) <= Square(r)) res &= true;
        else res = false;
    }
...
float binsrc() {
    float ng = 0; // 解が必ず存在しない最大値
    float ok = LONG_LONG_MAX;
