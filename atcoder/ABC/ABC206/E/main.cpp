...
void enum_coprimes_with_stern_brocot_tree(int N, int a, int b, int x, int y)
{
  ...
  if (d <= N) {
    ...
    enum_coprimes_with_stern_brocot_tree(N, n, d, x, y);
  }
}
   int L, R;
int ans;
void enum_coprimes_with_reverse_euclid(int N, int a, int b)
{
  for (int d = a+b; d <= N; d += b) {
      int x = b;
      int y = d;
      if (x < L and y < L) break;
      if (x > y) swap(x, y);
      if (L <= x and x <= R) {
         int l = L / y;
         int r = R / y;
         int sub = max(r - l + 1, 0LL);
         ans += sub;
      }
      enum_coprimes_with_reverse_euclid(N, b, d);
  }
}
