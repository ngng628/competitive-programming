...
struct Solver {
   Solver() = default;
   void solve() {
      auto [b, K, sx, sy, gx, gy] = sc.nextAi<6>();
      auto on = [b](int x, int y) {
         return y % b == 0 and x % b == 0;
      };
      int manh = abs(sx - gx) + abs(sy - gy);
      if (b == 1 or K == 1) {
         cout << manh << endl;
      }
      else if (on(sx, sy) and on(gx, gy)) {
         if (sx == gx and sy == gy) {
            cout << 0 << endl;
         }
         else if (sx == gx or sy == gy) {
            cout << manh + K + 1 << endl;
         }
         else {
            cout << manh - 2*K + 2;
