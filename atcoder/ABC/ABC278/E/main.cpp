...
int32 main() {
   auto [h, w, n, box_h, box_w] = sc.nextAi<5>();
   auto a = sc.nextVvi(h, w, -1);

   vvvi b(n, vvi(h, vi(w, 0)));
   rep (i, h) {
      rep (j, w) {
         b[a[i][j]][i][j] += 1;
      }
   ...
   vvvi csums(n, vvi(h + 1, vi(w + 1, 0)));
   rep (x, n) {
      vvi& csum = csums[x];
      rep (i, h) {
         rep (j, w) {
            csum[i + 1][j + 1] = csum[i][j + 1] + csum[i + 1][j] - csum[i][j] + b[x][i][j];
         }
      }
   }

   auto query = [&csums](int x, int i1, int i2, int j1, int j2) {
      return csums[x][i2][j2] - csums[x][i1][j2] - csums[x][i2][j1] + csums[x][i1][j1];
   };
   ...
   rep (i, h - box_h + 1) {
      rep (j, w - box_w + 1) {
         ...
         rep (x, n) {
            int acc = 0;
            acc += query(x, 0, i, 0, w);
            acc += query(x, i, i + box_h, 0, j);
            acc += query(x, i, i + box_h, j + box_w, w);
            acc += query(x, i + box_h, h, 0, w);
            if (acc) {
               ans += 1;
            }
         }
