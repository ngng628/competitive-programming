...
struct S{
   S() = default;
   S(long long v, int s) : value(v), size(s) {}
   long long value;
   int size;
};
using F = long long;
...
F id(){ return ID; }
using SegmentTree = atcoder::lazy_segtree<S, op, e, F, mapping, composition, id>;
...
int32 main() {
   ...
   rep (q) {
      ...
      if (t == 1) {
         ...
         auto i = sc.nextInt(-1);
         auto x = sc.nextInt();
         auto value = seg.get(i).value;
         seg.set(i, S(value + x, 1));
      }
      else {
         auto i = sc.nextInt(-1);
