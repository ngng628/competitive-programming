...
int32 main() {
   ...
   if (n == K) {
   ...
   else if (isodd(K) and all_of(all(a), [](int ai){ return ai < 0; })) {
      rsort(a);
      mint ans = 1;
      int org = 1;
      rep (i, K) {
      ...
      if (sgn == -1) {
         ...
         int max_pos = nil;
         rep (i, K, n) {
            if (max_neg == nil and math::sgn(a[i]) <= 0) {
               max_neg = a[i];
            }
            if (max_pos == nil and math::sgn(a[i]) >= 0) {
               max_pos = a[i];
            }
         }

         if (max_neg == nil or abs(min_pos) == nil) {
            ans /= min_neg;
            ans *= max_pos;
         }
         else if (max_pos == nil or abs(min_neg) == nil) {
            ans /= min_pos;
            ans *= max_neg;
         }
         else {
            //  / min_neg * max_pos
            //  / min_pos * max_neg
            if (max_neg * min_neg < max_pos * min_pos) {
               ans /= min_neg;
               ans *= max_pos;
            }
            else {
