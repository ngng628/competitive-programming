...
struct Solver {
   ...
   void solve() {
      ...
      auto sqrt_x = floor_sqrt(X);

      vec<int> dp(600000, 0);
      deque<bool> wj(600000, true);
      auto rec = Fix([&](auto&& Recall, int x) -> int {
         if (not wj[x]) return dp[x];
         if (x == 1) {
            wj[x] = false;
            return dp[x] = 1;
         }
      ...
      int ans = 0;
      for (int i = 1; i*i <= sqrt_x; ++i) {
         int low = i * i;
         int up = sqrt_x;
         ans += rec(i) * max<int>(up - low + 1, 0);
      }
