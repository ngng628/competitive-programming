...
dont_use = [] of {Int32, Int32}
n.to_i32.times do |i|
  if s[i].ascii_alphanumeric?
    next
  elsif s[i] == '('
    stack << i
  elsif s[i] == ')'
...
cur = 0
n.times do |i|
  if ( cur < dont_use.size && !i.in?(dont_use[cur][0]..dont_use[cur][1])) || cur == dont_use.size
    print s[i]
  end
  while cur < dont_use.size && i >= dont_use[cur][1]
    cur += 1
  end
end
puts ""
