...
int32_t main() {
   int N, C;
   cin >> N >> C;
   vec<tuple<int, int, int>> stc(N);
   rep (i, N) {
      cin >> get<0>(stc[i]) >> get<1>(stc[i]) >> get<2>(stc[i]);
      get<2>(stc[i])--;
   }

   vec<Imos> imos(C, Imos(1e5));
   for (auto [s, t, c] : stc) imos[c].add(s, t);
   vec<int> tmp(1e5 + 1);
   rep (i, C) {
      auto res = imos[i].get();
      for (int& r : res) r = (bool)r;
      rep (i, 1e5 + 1) tmp[i] += res[i];
   }

   print(*max_element(all(tmp)));
}