...
int e() { return oo; }
using SegmentTree = atcoder::segtree<int, op, e>;

int32 main() {
   auto [n, Q] = sc.nextPii();
   auto [a, b] = sc.nextPairViVi(n, 0, -1);
   auto query = sc.nextVecPii(Q, -1, -1);

   vec<multiset<int>> ms(2e5);
   rep (i, n) {
      ms[b[i]].insert(a[i]);
   }
   ...
   rep (i, 2e5) {
      if (not ms[i].empty()) {
         seg.set(i, *prev(ms[i].end()));
      }
   }

   for (auto [c, d] : query) {
      ms[b[c]].erase(ms[b[c]].find(a[c]));
      ms[d].insert(a[c]);

      if (not ms[b[c]].empty()) seg.set(b[c], *prev(ms[b[c]].end()));
      else seg.set(b[c], e());
      seg.set(d, *prev(ms[d].end()));
