...
t = s.map_with_index{ |si, i| {si, i} }.sort!

def lcp(s : String, t : String)
...
ans = [Int32::MIN] * n
t.each_with_index do |(si, index), i|
  chmax(ans[index], { lcp(t[i][0], t[i - 1][0]) }.min) if i >= 1
  chmax(ans[index], { lcp(t[i][0], t[i + 1][0]) }.min) if i + 1 < n
end
