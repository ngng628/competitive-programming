...
m.times do
  ...
  graph[u] << v
  graph[v] << u
  bfs.add_edge(u, v, directed: true)
  if ut.equiv?(u, v)
    puts "No"
    exit
  end
...
odist = bfs.shortest_path(start: 0)
root = odist.index(odist.max).not_nil!

dist = bfs.shortest_path(start: root)
puts ut.groups.size==1 ? "Yes" : "No"
