...
# endif
    ...
    inline Array<String> ReadStringArray(size_t n) { return ReadArray_impl<String>(n); }
...
//
////////////////////////////////////////////////////////////////
static const int64 MOD9 = 1e9 + 7;

int64 cnt;
int64 ans;
void rep(int64 x, int64 y)
{
    if (x <= 0 or y <= 0)
    {
        return;
    }
    auto tmp3 = [](int64 _x, int64 _y){return (_x + _y) % 3 == 0;};
    auto tmp4 = [](int64 _x, int64 _y){if (_x > _y)swap(_x, _y); return _y / _x <= 2;};
    auto tmp = [=](int64 _x, int64 _y){return tmp3(_x, _y) and ((_x==_y and _x%3==0) or tmp4(_x, _y));};
    auto tmp2 = [](int64 _x, int64 _y){return _x > 0 or _y > 0;};
    if (tmp(x, y)) // 巡れる
    {
        if (tmp2(x-1, y-2) and tmp(x - 1, y - 2) and tmp2(x-2, y-1) and  tmp(x - 2, y - 1))
        {
            ans = (ans * 2) % MOD9;
            rep(x-1, y-2);
            rep(x-2, y-1);
        }
        else if (tmp2(x-1, y - 2) and tmp(x - 1, y - 2))
        {
            rep(x-1, y-2);
        }
        else if (tmp2(x-2, y - 1) and tmp(x - 2, y - 1))
        {
            rep(x-2, y-1);
        }
        else
...
static const int64 MAX_X = (int64)1e6+1;
static const int64 MAX_Y = (int64)1e6+1;
...
int main()
{
    ...
    if ((X + Y) % 3 == 0 and (X / Y == 2 or Y / X == 2 or (X==Y and X%3==0))) // 巡れる
    {
        rep(X, Y);
        Print << ans;
    }
