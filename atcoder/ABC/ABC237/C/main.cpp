...
vec<pair<typename iterator_traits<InputIterator>::value_type, int>>
RunLengthEncoding(InputIterator first,InputIterator last) {
   using type = typename iterator_traits<InputIterator>::value_type;
   vec<pair<type, int>> res;
   for (auto it = first; it != last; ++it) {
      if (res.empty() or res.back().first != *it) res.eb(*it, 1);
...
int32 main() {
   ...
   auto code = RunLengthEncoding(all(s));
   if (code.back().first == 'a') {
      int less = [&](){
         if (code[0].first != 'a') return code.back().second;
         return max(0ll, code.back().second - code[0].second);
      }();
      rep (less) s.push_front('a');
   }
