...
n, k, q = ints
lower = AATreeMultiset.new([0_i64] * (n - k))
higher = AATreeMultiset.new([0_i64] * k)
...
ans = String.build { |io|
  q.times do
    ...
    if lower.includes?(from)
      ...
      sum = sum - from + lower.last
      higher.delete(from)
      higher << lower.last
      lower.delete_at(lower.size - 1)
    end

    if !higher.less_index(to).nil?
      sum = sum - higher.first + to
      lower << higher.first
      higher.delete_at(0)
      higher << to
    else
    ...
    raise "ピンチ" if lower.size + higher.size != n
