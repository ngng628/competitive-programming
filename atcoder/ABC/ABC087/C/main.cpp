...
{
    int32 N = ReadInt();
    Array<Array<int32>> A(2, Array<int32>(N, 0));
    for (int32 y : step(2))
    {
        for (int32 x : step(N))
        {
    ...
    Array<int32> sums0;
    sums0.push_back(A[0][0]);
    Array<int32> sums1;
    sums1.push_back(A[1][0]);

    for (int32 i : Range(1, N-1))
    {
        sums0.push_back(sums0[i-1] + A[0][i]);
        sums1.push_back(sums1[i-1] + A[1][i]);
    }

    int32 m = -1;
    for (int32 i : step(N))
    {
        chmax(m, sums0[i] + (*(sums1.end() - 1)) - sums1[i] + A[1][i]);
    }
