...
private:
   ...
   void bfs(int v, const DP& dp_p, int p = -1) {
      ...
      rep (i, deg) {
         dp_l[i + 1] = dp_l[i] * dp[v][i];
      }
      rrep (i, deg) {
         dp_r[i] = dp_r[i + 1] * dp[v][i];
      }
...
struct Monoid {
   int val;
   static Monoid e() { return Monoid{-1}; }
};
Monoid operator *(const Monoid& a, const Monoid& b) { return Monoid{max(a.val, b.val)}; }
Monoid connect(const Monoid& a){ return Monoid{a.val + 1}; }
...
int32_t main() {
   ...
   Rerooting<Monoid, connect> reroot(n, edges);

   for (int i = 0; i < n; i++) {
      cout << "頂点" << i + 1 << ": " << reroot.ans[i].val << endl;
   }
}