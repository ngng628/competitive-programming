...
int32 main() {
   ...
   auto [n, m, graph] = sc.nextGraph();

   vvi dp(n, vi(BitOperations::Bit(n), oo));
   rep (v, n) dp[v][0] = 0;
   rep (16) {
      rep (S, BitOperations::Bit(n)) {
         rep (u, n) {
            for (int v : graph[u]) {
               chmin(dp[v][S ^ BitOperations::Bit(v)], dp[u][S] + 1);
               chmin(dp[u][S ^ BitOperations::Bit(u)], dp[v][S] + 1);
            }
         }
      }
   }
