...
bool isOK(float sx, float sy, float gx, float gy, float x) {
   auto f = [=](float e) {
      float dx = x - sx;
      float dy = sy;
      float a = dy / dx;
   ...
   if (f(gx) >= gy) return true;
   else return false;
}

float binary_search(float sx, float sy, float gx, float gy) {
   float ok = sx;  // 答えはこれ以上
   float ng = gx;  // 答えはこれ未満

   while (abs(ok - ng) > eps) {
      float mid = (ok + ng) / 2.0; 
      if (isOK(sx, sy, gx, gy, mid)) ok = mid;
      else ng = mid;
   }
...
int32_t main() {
   float sx, sy, gx, gy;
   cin >> sx >> sy >> gx >> gy;
   float ans = binary_search(sx, sy, gx, gy);
   print(ans);
}
