...
struct LimitedPriorityQueue {
   LimitedPriorityQueue() = default;
   LimitedPriorityQueue(int n) : lim(n) {}

   void push(int x) {
      que.push(x);
      if (len(que) > lim) que.pop();
   }

   int kth(int k) {
      vi buff;
      while (not que.empty()) {
         buff.push_back(que.top());
         que.pop();
      }
      for (auto b : buff) que.push(b);
...
LimitQueue merge(LimitQueue a, LimitQueue b) {
   if (len(a) < len(b)) swap(a, b);
   while (not b.que.empty()) {
      a.push(b.que.top());
      b.que.pop();
   }
   while (len(a) > a.lim) {
...
int32 main() {
   ...
   vvi graph(n);
   rep (i, n - 1) {
      auto [a, b] = sc.nextPii(-1, -1);
      graph[a].push_back(b);
      graph[b].push_back(a);
   }
   auto isleaf = [&graph](int v) { return len(graph[v]) == 1 and v != 0; };
   vec<pii> query = sc.nextVecPii(Q, -1, -1);

   // 頂点 v で k 番目に大きい値
   vec<LimitQueue> dp(n);
   auto rec = [&](auto&& rec, int v, int par) -> void {
      if (isleaf(v)) {
         dp[v].push(x[v]);
         return;
      }
