...
int32 main() {
   ...
   map<int, vi> mp;
   rep (mask, 1, BitOperations::Bit(min<int>(n, 12))) {
      vi seq;
      rep (i, n) {
         if (BitOperations::Stand(mask, i)) seq.push_back(i);
      }

      int sum = accumulate(seq, int(0), [&a](int acc, int i){ return acc + a[i]; }) % 200;
      if (mp.find(sum) == mp.end()) {
         ...
         cout << "Yes" << '\n';
         cout << len(mp[sum]);
         for (int b : mp[sum]) cout << ' ' << b + 1;
         cout << '\n';
         cout << len(seq);
         for (int c : seq) cout << ' ' << c + 1;
         cout << '\n';
         return 0;
      }
   }
