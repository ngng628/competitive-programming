...
int32 main() {
   ...
   vec<pair<pii, pii>> ans;
   auto move = [&](const pii& from, const pii& to) {
      a[from.first][from.second] -= 1;
      a[to.first][to.second] += 1;
      ans.push_back(make_pair(make_pair(from.first + 1, from.second + 1), make_pair(to.first + 1, to.second + 1)));
   };
   rep (i, h) {
      rep (j, w) {
         if (i == h - 1 and j == w - 1) {
            ...
            if (isodd(a[i][j])) {
               move(make_pair(i, j), make_pair(i + 1, j));
            }
         }
         else {
            if (isodd(a[i][j])) {
               move(make_pair(i, j), make_pair(i, j + 1));
            }
         }
      }
   }
