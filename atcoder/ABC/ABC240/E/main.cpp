...
int32 main() {
   auto [n, _m, graph] = sc.nextTree();
   auto isleaf = [&graph](int v) { return len(graph[v]) == 1 and v != 0; };

   int idx = 1;
   vi L(n), R(n);
   auto dfs = [&](auto&& dfs, int subroot, int par) {
      if (isleaf(subroot)) {
         L[subroot] = R[subroot] = idx;
         idx += 1;
         return;
      }
      ...
      for (int child : graph[subroot]) {
         ...
         dfs(dfs, child, subroot);
         chmin(mn, min(L[child], R[child]));
         chmax(mx, max(L[child], R[child]));
      }
      L[subroot] = mn;
      R[subroot] = mx;
   };
