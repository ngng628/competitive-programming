...
int main()
{
    ...
    queue<int32> que;
    que.push(0);
    Array<Array<int32>> color(N, Array<int32>(N, -1));
    color[jun[0]][junb[0]] = 0;
    int32 max_index = -1;
    
    while (!que.empty())
    {
        int32 now = que.front(); que.pop();
        for (auto i : step(graph[jun[now]].size()))
        {
            if (color[jun[now]][i] >= 0)
            {
                continue;
            }
            que.push(graph[jun[now]][junb[i]]);
            color[jun[now]][i] = color[jun[now]][i] >= i ? i+1 : i;
            chmax(max_index, color[jun[now]][i]+1);
        }
    }
    Print << max_index;
    for (int32 i = 0; i < jun.size(); ++i)
    {
        Print << color[jun[i]][junb[i]];
    }
