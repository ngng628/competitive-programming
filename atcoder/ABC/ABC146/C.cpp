...
int main()
{
    ...
    {
        long double middle = (long double)(left + right) / 2.0;
        long double ans = A*(int64)middle + B*((int64)log10(middle)+1);
        if (ans <= (long double)X)
        {
            left = middle;
        }
        else
    ...
    int64 ans = A*(int64)right + B*((int64)log10(right)+1);
    if (ans <= X)
    {
        Print << (int64)right;
    }
