...
int32 main() {
   ...
   auto b = sc.nextVi(n);
   auto Q = sc.nextInt();
   vec<array<int, 3>> queries(Q);
   rep (i, Q) {
      auto [x, y] = sc.nextPii(-1, -1);
      queries[i] = { x, y, i };
   }

   // sizes[i] = [0, i] までの集合の大きさ
   auto sizes = [&](const vi& a) {
      vector<int> res(n);
      set<int> st;
      rep (i, n) {
         st.insert(a[i]);
   ...
   auto a_sizes = sizes(a);
   auto b_sizes = sizes(b);
   ...
   vb ans;
   {
      ...
      ans.push_back(true);
      while (true) {
         while (i < n and ast2.count(a[i])) {
            i += 1;
         }
         while (j < n and bst2.count(b[j])) {
            j += 1;
         }
         ...
         if (not bst.count(aa) and aa != bb) {
            ast.insert(aa);
         }
         if (not ast.count(bb) and aa != bb) {
            bst.insert(bb);
         }
         if (bst.count(aa)) {
            bst.erase(bst.find(aa));
         }
         if (ast.count(bb)) {
            ast.erase(ast.find(bb));
         }
         i += 1;
         j += 1;

         bool ok = !len(ast) or !len(bst);
         ans.push_back(ok);
      }
   }
