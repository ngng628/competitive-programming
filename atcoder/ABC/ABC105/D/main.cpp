...
int32 main() {
   ...
   modint::set_mod(m);
   vec<modint> a(n);
   for (auto& e : a) e = scan<int>();

   StaticRangeSum<modint> cumsum(a);
   map<int, int> mp;
   reps (i, len(cumsum.sums) - 1) {
      mp[cumsum.sums[i].val()] += 1;
   }

   int ans = 0;
   for (auto [key, value] : mp) {
      if (key == 0) ans += value*(value - 1) / 2 + value;
      else ans += value*(value - 1) / 2;
   }
