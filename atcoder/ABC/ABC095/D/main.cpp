...
struct Solver {
   ...
   int solve(int n, int C, vi x, vi v) {
      vi bias(n, 0);
      {
         int now = 0;
         rep (i, n) {
            int diff = i == 0 ? C - x[n - 1] : C - x[n - i - 1] - (C - x[n - i]);
            now += v[n - i - 1] - diff;
            if (i != 0) chmax(bias[n - i - 1], bias[n - i]);
            chmax(bias[n - i - 1], now);
         }
      }
      ...
      int now = 0;
      rep (i, n) {
         int diff = i == 0 ? x[i] : x[i] - x[i - 1];
         now += v[i] - diff;
         chmax(ans, now);

         int now2 = now - x[i];
         if (i != n - 1) now2 += bias[i + 1];
         chmax(ans, now2);
      }
...
int32 main() {
   auto [n, C] = sc.nextPii();
   auto [x, v] = sc.nextPairViVi(n);

   Solver superAI;
   int ans = superAI.solve(n, C, x, v);
   vi x2 = x, v2 = v;
   rep (i, n) {
      x2[i] = C - x[n - i - 1];
      v2[i] = v[n - i - 1];
   }
   chmax(ans, superAI.solve(n, C, x2, v2));
