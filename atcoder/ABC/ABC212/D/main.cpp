...
constexpr S INF = (1LL<<62)-(1LL<<31);

S op(S a, S b){ return std::min(a, b); }
S e(){ return INF; }
S mapping(F f, S x){ return min(f+x, int(1e15)); }
F composition(F f, F g){ return min(f+g, int(1e15)); }
F id(){ return 0; }
...
int32_t main() {
   ...
   reps (query, Q) {
      ...
      if (q == 1) {
         ...
         int x;
         cin >> x;
         tree.apply((int)(0), Q_MAX + 2, x);
      }
      else {
         int mn = tree.all_prod();
         global = mn;
         print(mn);
         int p = tree.max_right<g>(0);
         tree.set(p, e());
      }
   }
}
