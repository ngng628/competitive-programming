...
vi to_bin(int x, int width = 0) {
   vi res;
   for (; x; x >>= 1) res.pb(x & 1);
   while (len(res) < width) res.pb(0);
   // reverse(all(res));
   return res;
}
...
struct Solver {
   ...
   void solve() {
      ...
      // 桁数揃える
      while (len(bs) > len(ba)) ba.push_back(0);
      const int n = len(bs);

      // dp[i][c] := i 桁目に 桁上りしている状態が 可能 / 不可能
      ddb dp(n + 1, db(2, false));
      dp[0][false] = true;
      rep (i, n) {
         if (ba[i] == 1) {
            // x, y == 1 確定
            ...
            if (bs[i] == 0) {
               ...
               dp[i + 1][false] = dp[i][false] | dp[i][true];
               dp[i + 1][true] = false;
            }
         }
      }
      puts(dp[n][false] ? "Yes" : "No");
   }
};
