...
struct Solver {
   Solver() = default;
   bool solve(int h, int w, const array<int, 3>& abc) {
      int sum = accumulate(abc, int(0), [&h](int acc, int t) {
         return acc + ceil(t, h);
      });
...
int32 main() {
   auto [h, w] = sc.nextPii();
   auto abc = sc.nextAi<3>();
   sort(abc);

   bool ok = false;
   do {
      ok |= solver.solve(h, w, abc);
      ok |= solver.solve(w, h, abc);
   } while(next_permutation(abc));
