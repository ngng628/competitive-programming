...
int main()
{
    int64 N = ReadInt64(), D = ReadInt64(), A = ReadInt64();
    Array<int64> X(N), H(N);
    Array<P> ps(N);
    for (int32 i : step(N))
    {
    ...
    for (int32 i : step(N))
    {
        for (int32 k = 0; k < i; ++k)
        {
            if (InRange(X[i], range[k].first, range[k].second))
            {
                H[i] -= memo[k];
            }
            else if (X[i] <= range[k].second)
            {
                break;
            }
        ...
        if (H[i] > 0)
        {
            cnt += (int32)ceil((double)H[i]/A);
            //ans = cnt*A;
        }
        memo[i] += cnt*A;

        range[i].first = X[i];
        range[i].second = X[i] + 2*D;
    }
