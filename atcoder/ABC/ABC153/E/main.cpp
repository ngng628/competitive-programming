...
static const int32 Amax = 1e4; // nの最大値
static const int32 Bmax = 1e4; // Wの最大値
...
void solve_dp2()
{
    ...
    for (int32 i = 0; i < N; ++i)
    {
        for (int32 h = 0; h <= H; ++h)
        {
            if (h >= A[i])
            {
                dp[i+1][h] = min(min(dp[i][h - A[i]] - B[i], dp[i+1][h - A[i]] - B[i]), dp[i][h]);
            }
            else
            {
                dp[i+1][h] = dp[i][h];
...
//int32 w[MAX_N+1], v[MAX_N+1];
    ...
    sort(ps.begin(), ps.end(), compare_by_b);
