...
{
    int32 K_num = ReadInt();
    String K = to_string(K_num);

    queue<String> que;

    for (int32 i : Range(1, 9))
    {
    ...
    while (!que.empty())
    {
        ...
        mp[stoll(now.c_str())] = 1;
        if (now.length() == 10)
        {
            continue;
        }

        char c = now[now.length() - 1];
        int32 i = ctoi(c);

        if (i == 0)
        {
            que.push(now + to_string(i));
            que.push(now + to_string(1+i));
        }
        else if (i == 9)
        {
            que.push(now + to_string(i));
