...
int main()
{
    ...
    if (K == 1)
    {
        int32 digit = N.length();
        Debug << "[Debug]digit: " << digit;
        Debug << "[Debug]ctoi(N[0]): "<< ctoi(N[0]);
        Print << (digit - 1)*10 + ctoi(N[0]) - digit + 1;
    }
    else if (K == 2)
    {
        for (int32 n : Range(1, 109))
        {
            int32 cnt = 0;
            for (int32 i : Range(1, n))
            {
        ...
        int32 digit = N.length();
        Print << 90 + (digit - 2)*10 + ctoi(N[0])-10 - digit-2;
    }
