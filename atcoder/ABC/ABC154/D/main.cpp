...
static const int64 Nmax = 200000;

int main()
{
    int64 N = ReadInt64(), K = ReadInt64();
    Array<int64> p = ReadInt64Array(N);
    Array<long double> sums(200000+2, 0);
    sums[0] = 0;
    ...
    ans = lst[0];
    for (int32 i = 1; i+K-1 < N; ++i)
    {
        lst[i] = lst[i-1] - (sums[p[i-1]] / p[i-1]) + (sums[p[i+K-1]] / p[i+K-1]);
        chmax(ans, lst[i]);
    }
