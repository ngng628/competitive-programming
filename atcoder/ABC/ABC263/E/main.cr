...
dp = DynamicRangeSum(ModInt).new [0.to_m] * (n + 1)
dp[n] = 0.to_m
(1..n-1).reverse_each do |i|
  dp[i] = ((a[i] + 1).to_m / a[i]) * ((1.to_m / (a[i] + 1) * dp[i+1..Math.min(i+a[i], n)]) + 1)
end
...
class DynamicRangeSum(T)
  ...
  def [](range : Range(Int | Nil, Int?)) : T
    left = (range.begin || 0)
    right = (range.end || @size) + (range.exclusive? ? 0 : 1)
    sum(left, right)
  end

  def []=(i : Int, x : T); add(i, x - sum(i, i + 1)) end
