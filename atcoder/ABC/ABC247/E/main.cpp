...
int32 main() {
   auto [n, x, y] = sc.nextAi<3>();
   auto a = sc.nextVi(n);

   vi x_pos(n + 1, oo); // left 以降（leftを含む）で x が初めて表れる位置
   rep (i, n) {
      int idx = oo;
      rep (j, i, n) {
         if (a[j] == x) {
      ...
      reps (j, i, min(idx, n)) x_pos[j] = idx;
      i = idx;
   }

   vi y_pos(n + 1, oo);  // left 以降（leftを含む）で y が初めて表れる位置
   rep (i, n) {
      int idx = oo;
      rep (j, i, n) {
         if (a[j] == y) {
      ...
      reps (j, i, min(idx, n)) y_pos[j] = idx;
      i = idx;
   }

   vi xgt_pos(n + 1, oo);  // cur 以降で、x を初めて上回る位置
   rep (i, n) {
      int idx = oo;
      rep (j, i, n) {
         if (a[j] > x) {
      ...
      reps (j, i, min(idx, n)) xgt_pos[j] = idx;
      i = idx;
   }

   vi ylt_pos(n + 1, oo);  // cur 以降で、y を初めて下回る位置
   rep (i, n) {
      int idx = oo;
      rep (j, i, n) {
         if (a[j] < y) {
      ...
      reps (j, i, min(idx, n)) ylt_pos[j] = idx;
      i = idx;
   }
   ...
   int ans = 0;
   rep (i, n) {
      if (x_pos[i] > xgt_pos[i]) continue;
      if (y_pos[i] > ylt_pos[i]) continue;

      int first = min(max(x_pos[i], y_pos[i]), n);
      int last = min(min(xgt_pos[i], ylt_pos[i]), n);

      ans += max<int>(last - first, 0);
   }
