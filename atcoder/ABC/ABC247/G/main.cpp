...
int32 main() {
   auto n = sc.nextInt();
   vec<array<int, 3>> humans;
   rep (i, n) {
      auto [a, b, c] = sc.nextAi<3>();
      a--, b--;
      humans.push_back({ a, b, c });
   }

   vi ans;
   auto univ_id = [](int i) { return i + 1; };
   auto subject_id = [](int i) { return 150 + i + 1; };
   reps (flow_limit, n) {
      constexpr int OFFSET = 1e9 + 10;
      constexpr int S = 321, T = 333;
      atcoder::mcf_graph<int, int> graph(334);
      rep (i, 150) {
         graph.add_edge(S, univ_id(i), 1, 0);
         graph.add_edge(subject_id(i), T, 1, 0);
      }
      for (auto [a, b, c] : humans) {
         graph.add_edge(univ_id(a), subject_id(b), 1, OFFSET - c);
      }

      auto [flow, cost] = graph.flow(S, T, flow_limit);
      if (flow < flow_limit) break;
      ans.push_back(flow_limit * OFFSET - cost);
   }
