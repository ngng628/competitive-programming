...
int32_t main() {
   ...
   set<pii> st;
   for (auto p : pos) st.insert(p);

   set<array<pii, 4>> rects;
   rep (i, n) {
      rep (j, n) {
         auto [x1, y1] = pos[i];
         auto [x2, y2] = pos[j];
         if (x1 == x2 or y1 == y2) continue;
         pii p3 = make_pair(x1, y2);
         pii p4 = make_pair(x2, y1);
         if (st.count(p3) and st.count(p4)) {
            array<pii, 4> tmp = {pos[i], pos[j], p3, p4};
            sort(all(tmp));
            rects.insert(tmp);
         }
      }
