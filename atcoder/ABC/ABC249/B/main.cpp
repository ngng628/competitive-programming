...
int32 main() {
   auto s = sc.nextWord();
   set<char> st;
   bool appeared_upper = false;
   bool appeared_lower = false;
   bool all_differ = true;;
   for (char c : s) {
      all_differ &= st.find(c) == st.end();
      appeared_lower |= islower(c);
      appeared_upper |= isupper(c);
      st.insert(c);
   }

   puts(all_differ && appeared_upper && appeared_lower ? "Yes" : "No");
}
