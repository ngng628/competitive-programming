...
template <class T>
vec<T> rev(vec<T> v) { reverse(v); return v; }

int32 main() {
   auto [n, K] = sc.nextPii();
   auto queries = sc.nextVecPii(n);
   if (queries[0].first != 1) {
      queries.insert(queries.begin(), { 1, 0 });
      n += 1;
   }
   ...
   int sum = 0;
   int cnt = 0;
   for (auto [t, y] : rev(queries)) {
      if (t == 1) {
         chmax(ans, y + sum);
         cnt += 1;
         if (K - cnt < len(minheap)) {
            if (minheap.empty()) break;
            int better = minheap.top();
            minheap.pop();
            sum += (-1) * better;
         }
      }
