...
mint operator"" _mod_P(unsigned long long n) { return n; }
using vm = vec<mint>;
using vvm = vec<vm>;
using vvvm = vec<vvm>;
...
int32 main() {
   ...
   auto dp = make_vector({ n + 10, n + 10 }, 0_mod_P);
   dp[0][0] = 1;
   rep (t_len, n) {
      rep (s_len, n) {
         mint n_ways = dp[t_len][s_len] * (t_len == 0 ? 26 : 25);
         int ratio = 1;
         int d = 1;
         while (ratio <= n - s_len) {
            int begin = s_len + ratio;
            int end = s_len + min(10*ratio, n - s_len + 1);
            dp[t_len + (1 + d)][begin] += n_ways;
            dp[t_len + (1 + d)][end] -= n_ways;
            ratio *= 10;
            d += 1;
         }
      }
      rep (i, 1, n + 10) dp[t_len + 1][i] += dp[t_len + 1][i - 1];
   }
