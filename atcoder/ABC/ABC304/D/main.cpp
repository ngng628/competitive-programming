...
int32 main() {
   auto [w, h] = sc.nextPii();
   auto n = sc.nextInt();
   auto berries = sc.nextVecPii(n);
   auto a_len = sc.nextInt();
   auto a = sc.nextVi(a_len);
   auto b_len = sc.nextInt();
   auto b = sc.nextVi(b_len);
   ...
   for (auto [x, y] : berries) {
      auto lx = *prev(upper_bound(a, x));
      auto rx = *upper_bound(a, x);
      auto dy = *prev(upper_bound(b, y));
      auto uy = *upper_bound(b, y);
      cnt[make_tuple(lx, rx, dy, uy)] += 1;
   }
