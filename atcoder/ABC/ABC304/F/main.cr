...
n = int
s = str.chars
bits = Array.new(n) { |i| s[i] == '#' }

divs = n.divisors.sort[0...-1]
ms = divs

ans = Mint.zero
memo = Array.new(ms.size + 1, Mint.zero)
ms.each_with_index do |m, index|
  acc = [true] * m
  0.step(to: n - 1, by: m) do |i|
    (i...i + m).each_with_index do |j, b|
      acc[b] &= bits[j]
    end
  end
  # pp! acc
  cnt = acc.count { |flag| flag }
  add = Mint.new(2) ** cnt
  (0...index).each do |i|
    v = ms[i]
    if m % v == 0
