...
int32 main() {
   auto [h1, w1] = sc.nextPii();
   auto a = sc.nextVvi(h1, w1);
   auto [h2, w2] = sc.nextPii();
   auto b = sc.nextVvi(h2, w2);

   constexpr int nil = -1;
   rep (mask, BitOperations::Bit(h1)) {
      int n_erases_h = 0;
      auto tmp2 = a;
      rep (i, h1) {
      ...
      rep (maskw, BitOperations::Bit(w1)) {
         auto tmp = tmp2;
         int n_erases_w = 0;
         rep (j, w1) {
            if (BitOperations::Stand(maskw, j)) {
               n_erases_w += 1;
               rep (i, h1) {
                  tmp[i][j] = nil;
               }
         ...
         if (h1 - n_erases_h == h2 && w1 - n_erases_w == w2) {
            int cur_i = 0;
            int cur_j = 0;
            bool ok = true;
            rep (i, h2) {
               rep (j, w2) {
                  while (tmp[cur_i][cur_j] == nil) {
                     cur_j += 1;
                     if (cur_j == w1) {
                        cur_i += 1;
                        cur_j = 0;
