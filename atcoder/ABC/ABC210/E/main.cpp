...
struct Solver {
   ...
   auto input() {
      auto [h, w, C] = sc.nextAi<3>();
      auto a = sc.nextVvi(h, w);
      return make_tuple(h, w, C, a);
   }
   ...
   int solve(int h, int w, int C, vvi a) {
      ...
      rep (i, h) rep (j, w) {
         ...
         if (i >= 1) chmin(dp[i][j], dp[i - 1][j]);
         if (j >= 1) chmin(dp[i][j], dp[i][j - 1]);
      ...
      rep (i, h) rep (j, w) {
         ...
         if (i >= 1) {
            chmin(ans, a[i][j] + C*(i + j) + dp[i - 1][j]);
         }
         if (j >= 1) {
            chmin(ans, a[i][j] + C*(i + j) + dp[i][j - 1]);
         }
         if (i >= 1 and j >= 1) {
            chmin(ans, a[i][j] + C*(i + j) + dp[i - 1][j - 1]);
         }
      }
...
int32 main() {
   Solver solver;
   auto [H, W, C, A] = solver.input();
   int ans = solver.solve(H, W, C, A);
   chmin(ans, solver.solve(W, H, C, rotated(A, 1)));
   cout << ans << endl;
}
