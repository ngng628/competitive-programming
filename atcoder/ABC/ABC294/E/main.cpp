...
struct Code {
   ...
   int length;
   Code() = default;
   Code(int v, int l) : value(v), length(l) {}
};
...
int32 main() {
   ...
   // 入力
   // ───────────────────────────────
   auto [l, n, m] = sc.nextAi<3>();
   array<vec<Code>, 2> codes;
   codes[0].resize(n);
   codes[1].resize(m);
   vector<int> lens(m);
   rep (i, n) cin >> codes[0][i].value >> codes[0][i].length;
   rep (i, m) {
      cin >> codes[1][i].value >> codes[1][i].length;
      lens[i] = codes[1][i].length;
   }

   // ───────────────────────────────
   // 解く
   // ───────────────────────────────
   StaticRangeSum<int> csum(lens);
   ...
   //    while (ng - ok2 > 1) {
   //       int mid = (ok2 + ng) / 2;
   //       cout << "csum: " << mid << ", " << csum.get(0, mid) << endl;
   //       if (csum.get(0, mid) <= r) ok2 = mid;
   //       else ng = mid;
   //    }
   ...
   rep (i, n) {
      // [l, r)
      // int l = cur;
      int r = cur + codes[0][i].length;
      int x = codes[0][i].value;

      int same = 0;
      while (j < m && cur2 + codes[1][j].length < r) {
         if (codes[1][j].value == x) {
            same += codes[1][j].length;
         }
         cur2 += codes[1][j].length;
         j += 1;
      }
      ...
      // cout << "cur2: " << cur2 << endl;
      // cout << "same: " << same << endl;
      codes[1][j].length -= r - cur2;
      cur2 = r;


      cur += codes[0][i].length;
   ...
   // while (true) {
   //    // 領域がかぶってないならスキップ
   //    while (index[1] < lim[1] && cur[1] + codes[1][index[1]].length < cur[0]) {
   //       cur[1] += codes[1][index[1]].length;
   //       index[1] += 1;
   //    }
