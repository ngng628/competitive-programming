...
int32 main() {
   ...
   auto a = sc.nextVi(n);
   auto b = sc.nextVi(n);
   auto queries = sc.nextVecAi<4>(Q, { -1, -1, -1, -1 });

   SegmentTree seg1(n - 1), seg2(n - 1);
   rep (i, n - 1) {
      seg1.set(i, abs(b[i] - b[i + 1]));
      seg2.set(i, abs(a[i] - a[i + 1]));
   }

   for (auto [h1, h2, w1, w2] : queries) {
      int gw = seg1.prod(w1, w2);
      int gh = seg2.prod(h1, h2);
      int ge = a[h2] + b[w2];
      int g = gcd(gcd(gw, gh), ge);
      cout << g << '\n';
   }
}
