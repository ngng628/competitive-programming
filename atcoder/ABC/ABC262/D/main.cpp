...
mint operator"" _mod_998244353(unsigned long long n) { return n; }
using vm = vec<mint>;
using vvm = vec<vm>;
using vvvm = vec<vvm>;

int calc_mod(int a, int mod) {
...
int32 main() {
   ...
   auto dp = make_vector({n + 1, n + 1, n + 1}, 0_mod_998244353);
   rep (mod, n + 1) dp[0][mod][0] = 1;
   rep (i, n) {
      auto dp2 = make_vector({n + 1, n + 1, n + 1}, 0_mod_998244353);
      reps (j, 0, n) {
         reps (mod, 1, n) {
            rep (r, 0, mod) {
               dp2[j][mod][r] += dp[j][mod][r];
               if (j - 1 >= 0) {
                  dp2[j][mod][r] += dp[j - 1][mod][calc_mod(r - a[i], mod)];
               }
            }
         }
      }
