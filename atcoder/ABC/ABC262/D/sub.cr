...
ans = a.reduce(Array.new(n + 1){ |j| Array.new(n + 1){ Array.new(n + 1){ |r| j == 0 && r == 0 ? 1.to_m : 0.to_m } } }) do |dp, ai|
  dp2 = Array.new(n + 1){ Array.new(n + 1){ Array.new(n + 1){ 0.to_m } } }
  (0..n).each do |j|
    (1..n).each do |mod|
      mod.times do |r|
        dp2[j][mod][r] += dp[j][mod][r]
        dp2[j][mod][r] += dp[j - 1][mod][(r - ai) % mod] unless j < 0
      end
    end
  end
  dp2
end.each_with_index.sum{ |t, j| t[j][0] }
