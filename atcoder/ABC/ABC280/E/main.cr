...
cache = Array(ModInt?).new(n + 1){ nil }
dp = uninitialized Proc(Int64, ModInt)
dp = ->(hp : Int64) do
  return ModInt.zero if hp <= 0
  return cache[hp].not_nil! unless cache[hp].nil?

  cache[hp] = pt * (dp.call(hp - 2) + 1) + po * (dp.call(hp - 1) + 1)
