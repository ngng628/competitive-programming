...
T bisect(T ac, T wa, const F& f, int ts = 50) {
   while (ts--) {
      T wj = (ac + wa) / 2;
      (f(wj) ? ac : wa) = wj;
   }
...
int32 main() {
   ...
   float ans = bisect(float(1e6), float(0), [&](float r) {
      set<int> st;
      rep (i, K) {
         auto [s, t] = xy[a[i]];
         auto Ok = [&](float x, float y) -> bool {
            return sq(x - (float)s) + sq(y - (float)t) <= sq(r);
         };
         rep (j, n) {
            if (Ok(xy[j].first, xy[j].second)) {
               st.insert(j);
            }
         }
      }
