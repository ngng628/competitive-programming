...
struct Edge {
    int32 src, dst;
    Weight weight;
    Edge(int32 src, int32 dst, Weight weight) :
...
using Edges = Array<Edge>;
using Graph = Array<Edges>;

ostream& operator <<(ostream& os, Edge& dt) { return os << "[" << dt.src << "] " << "--" << dt.weight << "-->" << " [" << dt.dst << "]"; }

int main()
{
    int64 V = ReadInt64(); // 頂点
    int64 E; E = V - 1;  // 辺 = 頂点-1
    ...
    // この木の根は 頂点0 であることが保証されている
    map<int64, int64> cnts;
    cnts[0] = p[0];
    Array<int32> visited(V, false);

    queue<Edge> que;
    visited[0] = true;
    for (int64 i : step(len(graph[0])))
    {
        que.push(Edge(graph[0][i]));
    }
    ...
    while (!que.empty())
    {
        ...
        cerr << "now : " << now << endl;
        cnts[now.dst] += cnts[now.src] + p[now.dst];
        visited[now.dst] = true;
        for (int64 i : step(len(graph[now.dst])))
        {
            if (visited[graph[now.dst][i].dst]) { continue; }
            Edge next = graph[now.dst][i];
            que.push(next);
        }
    }
