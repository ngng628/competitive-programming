...
int main()
{
    ...
    Array<int64> L = ReadInt64Array(N);
    ...
    {
        for (auto k : Range(i+1, N-1))
        {
            // 二分探索でL[i] + L[k]以上の最初の要素のイテレータを見つけ出す
            auto itr = std::lower_bound(L.begin(), L.end(), L[i] + L[k]);

            // L[i] + L[k]以上の辺では三角形を構成することはできないためNG
            // 以下のコードでL[i] + L[k]以上の辺の数を数えてる
            int64 NG_num = distance(itr, L.end());
