...
n = int
colors = ints -1.to_i64
integers = ints -1

bit = DynamicRangeSum.new([0_i64] * n)
sets = Array.new(n){ AATreeMultiset(Int64).new }
ans = colors.zip(integers).sum do |c, x|
  save = sets[c].size - sets[c].upper_bound_index(x)
  s = bit[x+1...n]
  bit.add(x, 1)
  sets[c] << x
  (s - save).to_i64
...
class DynamicRangeSum(T)
  ...
  def initialize(array : Array(T))
    ...
    @data = Array(T).new(@size, 0)
    array.each_with_index do |x, i|
      add(i, x)
    end
  end

  def [](i : Int) : T; sum(i, i + 1) end
  def [](l : Int, r : Int) : T; sum(l, r) end
  def [](range : Range(Int, Int)) : T
    sum(range.begin, range.exclusive? ? range.end : range.end + 1)
  end
...
class AATreeMultiset(T)
  ...
  def print_nodes : Array(T)
    res = Array(T).new
    return res unless @root
    dfs = uninitialized Proc(Node(T), Int32, Nil)
    dfs = ->(node : Node(T), h : Int32) do
      dfs.call(node.left.not_nil!, h + 1) if node.left
      puts ("    " * h) + "#{node.key}"
      dfs.call(node.right.not_nil!, h + 1) if node.right
      nil
    end
    dfs.call(@root.not_nil!, 0)
    res
  end
