...
int32 main() {
   ...
   map<pii, int> index;
   rep (i, m) {
      auto [a, b, c] = sc.nextAi<3>({ -1, -1, 0 });
      graph[a].emplace_back(b, c);
      graph[b].emplace_back(a, c);
      index[make_pair(a, b)] = i + 1;
      index[make_pair(b, a)] = i + 1;
   }

   vi dist(len(graph), oo);
   vi route(len(graph), -1);
...
struct Scanner {
   ...
   template<size_t N> vec<array<int, N>> nextVecAi(int n) const { vec<array<int, N>> r(n); rep (i, n) nextAi<N>(); return r; }
