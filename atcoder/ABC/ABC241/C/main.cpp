...
int32 main() {
   ...
   rep (i, n) {
      rep (j, n) {
         ...
         rep (delta, 6) {
            auto update = [&s, &over, &cnt](int cur, int i, int j) {
               if (over(i, j)) cnt[cur] = -oo;
               else if (s[i][j] == '#') cnt[cur] += 1;
            };
            update(0, i + delta, j);
            update(1, i, j + delta);
            update(2, i + delta, j + delta);
            update(3, i + delta, j - delta);
         }
         
         if (any_of(all(cnt), [](int x){ return x >= 4; })) {
            puts("Yes");
            return 0;
         }
      }
