...
constexpr array<pair<int, int>, 6> dydx6 = {{
   {-1, -1},
   {-1, 0},
   {0, -1},
   {0, 1},
...
int32 main() {
   ...
   vvi board(3000, vi(3000, 0));
   rep (n) {
      auto [x, y] = sc.nextPii(1001, 1001);
      board[x][y] = 1;
   }

   auto over = [](int y, int x) { return y < 0 or y >= 3000 or x < 0 or x >= 3000; };

   int ans = 0;
   vvb closed(3000, vb(3000, false));
   rep (i, 3000) {
      rep (j, 3000) {
         if (closed[i][j] or board[i][j] == 0?) continue;
         ans += 1;

         closed[i][j] = true;
         for (queue<pii> que({{i, j}}); not que.empty();){
            auto [i, j] = que.front();
            que.pop();
            for (auto [di, dj] : dydx6) {
               int ni = i + di, nj = j + dj;
