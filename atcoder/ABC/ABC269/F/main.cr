...
def sigma(first : ModInt, diff : ModInt, num : ModInt)
  num * (first * 2 + (num - 1) * diff) / 2
end
...
q.times do
  ...
  if (a + c).even?
    # 左上が生きている
    ms = sigma((a - 1).to_m * m, m.to_m * 2, ((b - a + 1 + 1) // 2).to_m) * ((d - c + 1 + 1) // 2)
    ns = sigma(c.to_m, 2.to_m, ((d - c + 1 + 1) // 2).to_m) * ((b - a + 1 + 1) // 2)

    a += 1
    c += 1
    ms2 = sigma((a - 1).to_m * m, m.to_m * 2, ((b - a + 1 + 1) // 2).to_m) * ((d - c + 1 + 1) // 2)
    ns2 = sigma(c.to_m, 2.to_m, ((d - c + 1 + 1) // 2).to_m) * ((b - a + 1 + 1) // 2)
    puts "#{ms + ns + ms2 + ns2}"
  else
    # 左上が消されている
    ms = sigma((a - 1).to_m * m, m.to_m * 2, ((b - a + 1 + 1) // 2).to_m) * ((d - c + 1 + 1) // 2)
    ...
    ms3 = sigma((a - 1).to_m * m, m.to_m, ((b - a + 1)).to_m) * ((d - c + 1))
    ns3 = sigma(c.to_m, 1.to_m, ((d - c + 1)).to_m) * ((b - a + 1))
    
    a += 1
    c += 1
    ms2 = sigma((a - 1).to_m * m, m.to_m * 2, ((b - a + 1 + 1) // 2).to_m) * ((d - c + 1 + 1) // 2)
    ns2 = sigma(c.to_m, 2.to_m, ((d - c + 1 + 1) // 2).to_m) * ((b - a + 1 + 1) // 2)
    puts "#{(ms3 + ns3) - (ms + ns + ms2 + ns2)}"
  end
end
