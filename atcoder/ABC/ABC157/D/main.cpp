...
int main()
{
    int32 N  = ReadInt32(), M = ReadInt32(), K = ReadInt32();
    Array<Array<int32>> graph(N);
    for (int32 i : step(M))
    {
        int32 a = ReadInt() - 1, b = ReadInt() - 1;
        graph[a].push_back(b);
        graph[b].push_back(a);
    }
    map<pair<int32, int32>, int32> block;
    for (int32 i : step(K))
    {
        int32 a = ReadInt() - 1, b = ReadInt() - 1;
        block[pair<int32, int32>(a, b)] = 1;
        block[pair<int32, int32>(b, a)] = 1;
    }

    Array<int32> endnodes(N, -1);
    queue<int32> q;
    q.push(0);
    
    while (!q.empty())
    {
        int32 now = q.front(); q.pop();
        int32 sz = len(graph[now]);
        if (sz == 0 or sz == 1)
        {
            endnodes[now] = 1;
        }
        ...
        for (int32 i : step(sz))
        {
            int32 next = graph[now][i];
            if (endnodes[next] != -1) // already visited
            {
                continue;
            }
    ...
    for (int32 human : step(len(nums)))
    {
        Array<int32> visited(N, 0);
        q.push(nums[human]);
        visited[nums[human]] = 1;
        while (!q.empty())
        {
            int32 now = q.front(); q.pop();
            int32 sz = len(graph[now]);
            for (int32 i : step(sz))
            {
                ...
                if (endnodes[next] == 1)
                {
                    if (block[pair<int32, int32>(nums[human], next)] != 1)
                    {
                        ans[nums[0]]++;
                    }
                }
