...
# dp[i][m][j] = i 番目までで、(m ? 上がる : 下がる) 状態で最後の数が j であるようなのジグザグな数列の数
# dp[i][j] = dp[i - 1][j - 1]

dp = Array.new(2){ DynamicRangeSum.new(a.max + 2, 0.to_m) }
dp[1][a[0]] = dp[0][a[0]] = 1.to_m
a.each_with_index do |ai, i|
  next if i == 0
  m = i % 2
  if m == 0
    dp[m][ai] = dp[m][ai] + dp[m ^ 1][...ai]
    dp[m ^ 1][ai] = dp[m ^ 1][ai] + dp[m][ai + 1...]
  else
    dp[m][ai] = dp[m][ai] + dp[m ^ 1][ai + 1...]
    dp[m ^ 1][ai] = dp[m ^ 1][ai] + dp[m][...ai]
  end
end
