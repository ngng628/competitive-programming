...
int32 H, W;
int32 dp[Hmax+1][Wmax+1];

Array<String> flip(Array<String> s,int32 x1,int32 y1,int32 x2,int32 y2)
{
...
{
    H = ReadInt(), W = ReadInt();
    Array<String> S = ReadWordArray(H);
    for (int32 y : step(H))
    {
        for (int32 x : step(W))
        {
    ...
    for (int32 x = 1; x < W; ++x)
    {
        dp[0][x] = dp[0][x-1] + ((S[0][x] == '#' and S[0][x-1] != '#') ? 1 : 0);
    }
    for (int32 y = 1; y < H; ++y)
    {
        dp[y][0] = dp[y-1][0] + ((S[y][0] == '#' and S[y-1][0] != '#') ? 1 : 0);
    }

    for (int32 y : Range(1, H-1))
    {
        for (int32 x : Range(1, W-1))
        {

            dp[y][x] = min(dp[y-1][x], dp[y][x-1]);
            if (S[y][x] == '#' and (S[y-1][x] != '#' or S[y][x-1] != '#'))
            {
                dp[y][x]++;
            }
        }
