...
# endif
	...
	struct Point
	{
		...
		constexpr Point(int32 _x, int32 _y) noexcept : x(_y), y(_x) {}
...
using P = pair<int32, int32>;
using PP = pair<P, int32>;
int32 H, W;

bool IsGoal(P pos)
{
  if (pos.first+1 == H and pos.second+1 == W)
  {
    return true;
  }
  else
...
bool isOK(P pos, const Array<String>& s)
{
...
int main()
{
    ...
    if (!isOK(P(0, 0), S))
    {
        que.push(PP(P(0, 0), ans + 1));
    }
    else
    {
        que.push(PP(P(0, 0), ans));
    }
    ...
    while (!que.empty())
    {
        ...
        if (IsGoal(now.first))
        {
            chmin(ans, now.second);
            continue;
        }

        {
            int32 xx = now.first.second + 1;
            if (xx < W)
            {
                P next = P(now.first.first, xx);
                if (!isOK(next, S))
                {
                    que.push(PP(next, now.second + 1));
                }
                else
                {
                    que.push(PP(next, now.second));
                }
            }
        }
        {
            int32 yy = now.first.first + 1;
            if (yy < H)
            {
                P next = P(yy, now.first.second);
                if (!isOK(next, S))
                {
                    que.push(PP(next, now.second + 1));
                }
