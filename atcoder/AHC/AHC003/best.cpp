...
int32_t main() {
   ...
   rep (q, query) {
      ...
      rep (i, len(path) - 1) {
         ...
         double new_cost = (clamp(ave_keisu, 1.0, 3.0)*ave + 3 * min(1.0, double(q) / 300 )*est + reliability*cost[a][b]) / (reliability + clamp(ave_keisu, 1.0, 3.0) + 3*min(1.0, double(q) / 300));
