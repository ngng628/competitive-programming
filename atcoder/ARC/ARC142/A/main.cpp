...
int32 main() {
   const auto tos = [](const int n) -> string { return to_string(n); };
   auto [n, K] = sc.nextPii();
   auto Ks = tos(K);

   bool can_rev = [](string s) {
      auto rev = s;
      reverse(rev);
      return stoll(s) <= stoll(rev);
   }(Ks);

   if (K % 10 == 0 or not can_rev) {
      puts("0");
      return 0;
   }
   ...
   string zeros = "";
   set<string> st;
   while (stoll(Ks + zeros) <= n) {
      st.insert(Ks + zeros);
      if (stoll(rev + zeros) <= n and can_rev) st.insert(rev + zeros);
      zeros.push_back('0');
   }
