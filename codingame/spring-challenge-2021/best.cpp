   ...
   rep (i, n_cells) {
      ...
      for (auto&& p : s.my_trees) {
         if (dist[i][p.cell_index] <= p.size and not p.is_dormant and s.me.sun >= s.my_n_sizes[0]) {
            if (s.nutrients > 10) pq.emplace(my_shadow, richness, shadow, d2, md2, p.cell_index, i);
            else pq.emplace(richness, my_shadow, shadow, d2, md2, p.cell_index, i);
         }
      }
   }
   while (not pq.empty()) {
...
string best_grow(const State& s) {
   ...
   for (auto&& t : s.my_trees) {
      ...
      auto f = [&s](int sz) {
         return (sz == 2 ? 7 : 2*sz + 1) + s.my_n_sizes[sz + 1];
      };
      int cost = f(t.size);
      ...
      if (n_my_completed == 0) {
         // 決め打ち
         if (s.my_n_sizes[3] == 0 and s.my_n_sizes[2] == 2 and s.my_n_sizes[1] == 1 and s.my_n_sizes[0] == 1) {
             pq.emplace(t.size == 1 ? 4 : t.size, INF - cost, flyness, cells[idx].richness, idx);
         }
         else if (s.my_n_sizes[3] - s.enemy_n_sizes[3] >= 1 and s.my_n_sizes[0] >= 1 and s.my_n_sizes[1] >= 1 and f(0) + f(1) + 1 <= s.me.sun) {
             pq.emplace(t.size == 1 ? 4 : t.size, INF - cost, flyness, cells[idx].richness, idx);
         }
         else if (s.day <= 4 and s.day <= 10) pq.emplace(t.size, INF - cost, flyness, cells[idx].richness, idx);
      ...
      else if (my_grow_day == 0) {
          if (s.me.sun + 8 <= s.enemy.sun and s.my_n_sizes[3] >= 4) pq.emplace(INF - cost, t.size, flyness, cells[idx].richness, idx);
          else pq.emplace(t.size, INF - cost, flyness, cells[idx].richness, idx);
      }
...
string best_complete(const State& s) {
   ...
   auto calc = [](const State& s, int dir, int day) {
      int shadow_direction = dir % 6;
      vi shadows(n_cells, 0);
      rep (i, n_cells) {
         int depth = min(s.trees[i].size + day, 3);
         int idx = i;
         rep (_, depth) {
            idx = cells[idx].neigh[shadow_direction];
            if (idx == -1) break;
            chmax(shadows[idx], s.trees[i].size);
         }
      }
      return shadows;
   };
   ...
   for (auto&& t : s.my_trees) {
      ...
      int shadow = 3*shadows[0][t.cell_index] + shadows[1][t.cell_index] + shadows[2][t.cell_index];
      pq.emplace(cells[t.cell_index].richness, shadow, t.cell_index);
   }
   while (not pq.empty()) {
      auto [_0, _1, idx] = pq.top(); pq.pop();
...
string rule_base(State s) {
   ...
   // 最後のターン
   ...
   else if (24 - day > s.my_n_sizes[3] or s.nutrients > 10) {
      // 種は一日一回まで
      bool should_complete = [&]() {
         ...
         bool ad = [&]() {
            ...
            if (s.me.score + 15 < s.enemy.score) return false;
            if (n_my_completed == 0) return false;
            if (s.me.sun < 8) return false;
            return true;
         }();

        //  次のターンもらえそうなsunの数
        int n_suns = [&s]() {
            int shadow_direction = (s.day + 1) % 6;
            vi shadows(n_cells, 0);
            rep (i, n_cells) {
                int depth = s.trees[i].size;
                int idx = i;
                rep (_, depth) {
                    idx = cells[idx].neigh[shadow_direction];
                    if (idx == -1) break;
            ...
            int res = 0;
            for (auto& t : s.my_trees) {
                if (shadows[t.cell_index] < t.size) res += t.size;
            }
            return res;
        }();

         if (n_enemy_completed == 0) return false;
         else if (my_complete_day >= 2 and s.nutrients > 8) return false;
         else if (n_my_completed == 0 and n_enemy_completed == 0 and s.me.sun >= 12 and s.my_n_sizes[3] >= 5 and s.my_n_sizes[3] - s.enemy_n_sizes[3] > 1) {
            return true;
         }
         else if (s.enemy.is_waiting or s.enemy.sun < 4) {
            // 相手が眠っているときがスキあり
            // 攻めろーー！！！！！！！
        ...
        //  else if (s.my_n_sizes[3] >= 5 and n_suns < 4 and s.me.sun >= 10) {
        //      return true;
        //  }
         // あんまり木が多すぎたら刈る
         else if (s.my_n_sizes[3] >= 5) {
            return true;
         }
         else if (n_my_completed != 0 and abs(s.me.score - s.enemy.score) <= 2 and abs(s.me.sun - s.enemy.sun) < 4 and s.my_n_sizes[3] >= 3) {
             return true;
         }
         // 相手が刈ってきたらこっちもなるべく n + 1 回刈る
            ...
            // 森の栄養やばかったら規制緩和
            if (s.nutrients * 3 + s.me.score < s.enemy.score + s.nutrients - 3) return true;
            if (s.me.sun + 6 <= s.enemy.sun) return false;
            // if (s.enemy.sun < 4 and n_my_completed + 1 >= n_enemy_completed) return false;
            return true;
         }
         return false;
      }();
      ...
      else if (my_seed_day == 0 and len(s.my_trees) <= 7) return best_seed(s);
   }
   else {
      // 終盤
      eprint("終盤");
      ...
      if (res != "WAIT") {
          Action act = Action(res);
          int idx = act.index[0];
          int plus = 2*(cells[idx].richness - 1) + s.nutrients;
          if (plus > 3) return res;
          else {
              res = best_grow(s);
...
int main() {
   ...
   while (true) {
      ...
      eprint("[行動の様子]");
      eprint("[o] 切った回数", n_my_completed, n_enemy_completed);
